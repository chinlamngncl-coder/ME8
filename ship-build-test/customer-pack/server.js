    ...
    try {
        if (process.env.FM_TRACE === '1') {
            return res.json({
                ...
                traceEnabled: true,
                traceEnvLocked: true,
                message: 'Trace locked on by FM_TRACE=1 in .env',
            });
        }
        const body = req.body || {};
...
function buildVoiceProfileForClient() {
    return bwcVoiceProfile.buildClientPayload(buildVoiceProfileDeps());
}
...
function startVideoForSosAlarm(camId) {
    ...
    function checkAndPullVideo() {
        ...
        setTimeout(checkAndPullVideo, 1000);
    }
...
function launchOutboundTalkCall(camId, socket) {
    ...
    log.media.info('voice call outbound-talk', { camId });
    mediaSession.startOutboundTalkCall(sip, {
        ...
        sipPort: SIP_PORT,
        onConnected: (id) => {
            emitBwcCallState(id, true, null, { via: 'outbound-talk' });
            auditLog.record('voice.call', Object.assign(
                { target: id, mode: 'outbound-talk' },
                auditVoiceCallActor(socket),
            ));
        },
        onFailed: (msg) => {
            pttVoiceCallCamId = null;
            emitBwcCallState(camId, false, msg || 'Outbound Talk call failed');
        },
    });
}
