...
function list(opts, displayNameFn) {
    const q = normalizeQueryOpts(opts);
    const result = siteDb.queryAudit(q);
    return {
        ...
        limit: q.limit,
        offset: q.offset,
        storeTotal: siteDb.countAudit ? siteDb.countAudit() : result.total,
    };
}

function listMeta() {
    const actions = siteDb.listDistinctAuditActions ? siteDb.listDistinctAuditActions() : [];
    return {
        ...
        storeTotal: siteDb.countAudit ? siteDb.countAudit() : 0,
    };
}
...
function exportCsv(opts, displayNameFn) {
