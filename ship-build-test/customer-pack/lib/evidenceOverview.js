...
function safeCatalogFiles(siteDb, evidenceRegistry, operatorErrorVoice, limit) {
    try {
        if (!siteDb.isReady()) {
            return {
                available: false,
        ...
        return {
            available: true,
            files: evidenceRegistry.listCatalog(limit || 500),
        };
    } catch (err) {
        return {
            available: false,
...
function build(ctx) {
    ...
    const catWrap = safeCatalogFiles(ctx.siteDb, ctx.evidenceRegistry, ctx.operatorErrorVoice, 500);
    const catalog = {
    ...
    if (!catWrap.available && catWrap.error) {
        ...
        try {
            const stats = ctx.siteDb.getDatabaseStats();
            if (stats) {
    ...
    const bwcData = ctx.loadBwcDevices();
