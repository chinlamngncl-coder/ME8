...
(function (global) {
    ...
    /** After SOS session ends — restore GTID 49 always-on PTT (not tied to ack or stop-video). */
    function restorePttAfterSosSessionClose() {
        if (!global.activeSosPttTeam) return;
    ...
    function updateMapPinStopButton(camId) {
        ...
        if (box) {
            if (pinDecoded) {
                ...
                if (streaming && !pinDecoded) syncMapPinStreamingOverlay(id);
            }
        }
        syncPinAudioUi(camId);
        syncPinCallUi(camId);
    ...
    function notifyStreamStopped() {
        ...
        getSlots().forEach((slotEl) => {
            const stage = slotEl.querySelector('.video-slot-stage');
            if (stage) {
                stage.innerHTML = '<span class="video-slot-empty">Stopped — press ▶</span>';
            }
            const st = slotEl.querySelector('.video-slot-status');
    ...
    function playMapPinVideoIfPopupOpen(camId, attempt, opts) {
        ...
        if (!root) {
            if (attempt < 20) {
                setTimeout(function () { playMapPinVideoIfPopupOpen(camId, attempt + 1, opts); }, 100);
            }
            return;
        }
        if (root.getAttribute('data-offline-popup') === '1') return;
    ...
    function onSosAlarm(data) {
        ...
        applySosAlarmUi(camId, slotIndex);
        playMapPinVideoForSos(camId, slotIndex);
        pendingSosVideo = { cameraId: camId, slotIndex: slotIndex };
        setTimeout(function () {
            if (!pendingSosVideo || pendingSosVideo.cameraId !== camId) return;
            pendingSosVideo = null;
            if (!wallHasPlayerForCam(camId)) {
