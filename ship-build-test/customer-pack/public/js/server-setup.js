...
(function (global) {
    ...
    function opMsg(data, err) {
        ...
        if (err && err.message && err.message !== 'op') return err.message;
    ...
    function openConfigPanel(mainTab) {
        ...
        if (cachedSettingsData) applySettingsPayload(cachedSettingsData);
        setOpen(true);
    ...
    function bindUi() {
        ...
        if (resetCancel) resetCancel.addEventListener('click', closeResetPwdDialog);
        if (resetSubmit) {
            resetSubmit.addEventListener('click', async () => {
                if (!resetPwdUserId) return;
                const newPass = document.getElementById('ss-reset-new-pass').value;
                const adminPass = document.getElementById('ss-reset-admin-pass').value;
                const errEl = document.getElementById('ss-reset-pwd-error');
    ...
    function bindSettingsAsideClicks() {
        ...
        aside.addEventListener('click', function (e) {
            if (e.target.closest('#server-setup-open')) {
                e.preventDefault();
                if (!trySettingsNavLock()) return;
                const openPanel = function () { openConfigPanel('server'); };
                if (isConfigUnlocked()) openPanel();
                else showGate(openPanel);
            }
        });
    }
    ...
    function openConfigTab(tab) {
        ...
        const openPanel = function () { openConfigPanel(mainTab); };
        const needsTech = mainTab === 'diagnostics' || mainTab === 'lab';
        const afterGate = function () {
            if (needsTech) {
                runWithTechAccess(openPanel, { onCancel: onTechGateCancel });
                return;
            }
            openPanel();
        };
    ...
    function init() {
        ...
        fetch('/api/auth/session', { credentials: 'same-origin' })
            ...
            .catch(function () { /* ignore */ });
        const prefetch = function () { scheduleSettingsRefresh(null, { tabExtras: false }); };
