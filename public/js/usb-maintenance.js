...
(function (global) {
    ...
    async function refresh() {
        ...
        try {
            ...
            const meta = (global.OperatorUI)
                ? OperatorUI.opMessage(err, null, 'errors.generic')
                : (err.message || tr('errors.generic'));
            setMeta(meta, true);
        }
    }
    ...
    async function runAction(action, destructive) {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok || !data.ok) {
                const msg = (global.OperatorUI)
                    ? OperatorUI.opMsg(data, null, 'errors.generic')
                    : tr('errors.generic');
                alert(msg);
                throw (global.OperatorErrorVoice
                    ? OperatorErrorVoice.attach(new Error('op'), data)
    ...
    async function launchVendorTool() {
        ...
        try {
            ...
            if (!res.ok || !data.ok) {
                const msg = (global.OperatorUI)
                    ? OperatorUI.opMsg(data, null, 'usbMaint.adbMissing')
                    : tr('usbMaint.adbMissing');
                alert(msg);
                throw (global.OperatorErrorVoice
                    ? OperatorErrorVoice.attach(new Error('op'), data)
                    : new Error(msg));
            ...
            const meta = (global.OperatorUI)
                ? OperatorUI.opMessage(err, null, 'usbMaint.adbMissing')
                : (err.message || tr('usbMaint.adbMissing'));
            setMeta(meta, true);
        }
    }
