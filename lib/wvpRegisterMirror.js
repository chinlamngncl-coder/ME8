...
function mirrorCfg() {
    const platformId = String(
        process.env.GB_PLATFORM_ID
        || process.env.FM_WVP_SERVER_ID
        || process.env.WVP_ID
        || '99999900002000000001'
    ).trim();
    const domain = String(
        process.env.GB_DOMAIN
        || process.env.FM_WVP_DOMAIN
        || process.env.WVP_DOMAIN
        || platformId.slice(0, 10)
        || '9999990000'
    ).trim();
    return {
