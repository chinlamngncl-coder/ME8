...
/* ALARM-MARKER-CONTRACT-V1 — same alias set + timestamptz cast as the Investigation timeline. */
async function alarmCamIdList(camId) {
    const set = new Set([String(camId)]);
    try {
...
async function loadAlarmsInRange(camId, fromIso, toIso) {
    const camIds = await alarmCamIdList(camId);
    const { rows } = await siteDb.query(
        `SELECT id, cam_id, event_type, occurred_at, note
         ...
         ORDER BY occurred_at::timestamptz ASC`,
        [camIds, fromIso, toIso]
    );
    return rows || [];
}
