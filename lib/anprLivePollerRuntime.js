...
/**
 * ANPR live grab cadence — ~6–7 FPS (150ms). Was 333ms (~3 FPS) → hardware idle look.
 * Hatch: FM_ANPR_GRAB_MS (100–2000). Busy path must not slow below grab floor.
 */
const _grabRaw = parseInt(process.env.FM_ANPR_GRAB_MS || '150', 10) || 150;
const GRAB_MS = Math.max(100, Math.min(2000, _grabRaw));
const _grabBusyRaw = parseInt(process.env.FM_ANPR_GRAB_MS_BUSY || String(GRAB_MS), 10) || GRAB_MS;
const GRAB_MS_BUSY = Math.max(100, Math.min(2000, _grabBusyRaw));
/** Drop still if ffmpeg grab wall time exceeds this (ms). */
const GRAB_BUDGET_MS = Math.max(400, Math.min(8000,
    parseInt(process.env.FM_ANPR_GRAB_BUDGET_MS || '1500', 10) || 1500));
/** Drop queued/grabbed frames older than this — LIFO newest only. */
const STALE_FRAME_MS = Math.max(800, Math.min(5000,
    parseInt(process.env.FM_ANPR_STALE_FRAME_MS || '2500', 10) || 2500));
/**
...
/** Child isolate mode — parent pushes live set / license / labels via IPC */
const IS_CHILD = String(process.env.FM_ANPR_POLLER_CHILD || '').trim() === '1';
/** External ingest service worker (separate process; same pushed-FLV contract as child). */
const IS_INGEST_WORKER = String(process.env.FM_ANPR_INGEST_WORKER || '').trim() === '1';
function usesPushedFlv() {
    return IS_CHILD || IS_INGEST_WORKER;
}
let childLiveCamIds = Object.create(null);
let childLicensed = false;
/** @type {Record<string, string>} camId → upstream FLV URL (from main; child has no WVP memory) */
let childLiveFlv = Object.create(null);
/** @type {Map<string, string>} */
const childLabels = new Map();
/** @type {Map<string, { lat: number, lon: number, at?: * }>} */
const childGps = new Map();
...
/**
 * Post-OCR text cooldown — plate_text → last_seen_ts.
 * Same string within PLATE_TEXT_COOLDOWN_MS → discard (shoot-first dedupe).
 * @type {Map<string, number>}
 */
const plateTextCooldown = new Map();
const PLATE_TEXT_COOLDOWN_MS = Math.max(1000, Math.min(120000,
    parseInt(process.env.FM_ANPR_PLATE_TEXT_COOLDOWN_MS || '10000', 10) || 10000));
/**
...
setInterval(() => {
    ...
    for (const [plateKey, ts] of plateTextCooldown.entries()) {
        if (now - ts > PLATE_TEXT_COOLDOWN_MS * 3) {
            plateTextCooldown.delete(plateKey);
        }
    }
...
function isAnprLicensed() {
    if (usesPushedFlv()) return !!childLicensed;
...
function isCamLive(camId) {
    ...
    if (!id) return false;
    if (usesPushedFlv()) {
        return !!childLiveCamIds[id] || !!childLiveFlv[id];
    }
...
/** Parent → child IPC (isolate mode only). */
function applyChildIpc(msg) {
    if (!msg || typeof msg !== 'object') return;
    if (msg.type === 'licensed') {
        childLicensed = !!msg.value;
        return;
    }
    if (msg.type === 'liveCams') {
        const next = Object.create(null);
        (Array.isArray(msg.ids) ? msg.ids : []).forEach((id) => {
            const s = String(id || '').trim();
            if (s) next[s] = true;
        });
        childLiveCamIds = next;
        return;
    }
    if (msg.type === 'liveFlv' && msg.map && typeof msg.map === 'object') {
        const next = Object.create(null);
        Object.keys(msg.map).forEach((k) => {
            const id = String(k || '').trim();
            const url = String(msg.map[k] || '').trim();
            if (id && url) next[id] = url;
        });
        childLiveFlv = next;
        return;
    }
    if (msg.type === 'labels' && msg.map && typeof msg.map === 'object') {
        Object.keys(msg.map).forEach((k) => {
            childLabels.set(String(k), String(msg.map[k] || k));
        });
        return;
    }
    if (msg.type === 'gps' && msg.map && typeof msg.map === 'object') {
        Object.keys(msg.map).forEach((k) => {
            const g = msg.map[k];
            if (g && Number.isFinite(Number(g.lat)) && Number.isFinite(Number(g.lon))) {
                childGps.set(String(k), {
                    lat: Number(g.lat),
                    lon: Number(g.lon),
                    at: g.at || null,
                });
    ...
    if (msg.type === 'setWatchSlots') {
        setWatchSlots(msg.socketId, msg.camIds);
        return;
    }
    if (msg.type === 'clearSocket') {
        clearSocket(msg.socketId);
        return;
    }
    if (msg.type === 'start') {
        start();
...
function childSend(payload) {
    if (!IS_CHILD || typeof process.send !== 'function') return;
    try { process.send(payload); } catch (_) { /* ignore */ }
}
...
function deviceLabel(camId) {
    if (usesPushedFlv() && childLabels.has(String(camId))) {
        return childLabels.get(String(camId)) || String(camId);
    }
...
function gpsFields(camId) {
    if (usesPushedFlv()) {
        const g = childGps.get(String(camId));
        if (g && Number.isFinite(Number(g.lat)) && Number.isFinite(Number(g.lon))) {
...
function vehicleEntriesFromTrackResult(result) {
    const list = [];
    const arr = result && Array.isArray(result.vehicles) ? result.vehicles : null;
    if (arr && arr.length) {
        for (let i = 0; i < arr.length; i++) {
            ...
            const w = Number(v.w);
            const h = Number(v.h);
            if (!Number.isFinite(w) || !Number.isFinite(h) || w < 4 || h < 4) continue;
            const b64 = v.vehicleJpegB64 || null;
            if (!b64 && !(result.vehicleJpegB64 && i === 0)) continue;
            list.push({
                bbox: {
                    x: Number(v.x) || 0,
                    y: Number(v.y) || 0,
                ...
                vehicleJpegB64: b64 || (i === 0 ? result.vehicleJpegB64 : null),
                sharpness: v.sharpness != null ? Number(v.sharpness) : Number(result.sharpness) || 0,
                label: v.label || null,
                make: v.make || (v.mmr && v.mmr.make) || null,
                model: v.model || (v.mmr && v.mmr.model) || null,
                color: v.color || (v.mmr && v.mmr.color) || null,
                mmrText: v.mmrText || (v.mmr && v.mmr.mmrText) || null,
                mmr: v.mmr || null,
            });
        }
    }
    if (!list.length) {
        ...
        if (bbox && result.vehicleJpegB64) {
            const vv = result.vehicle || {};
            const mmr0 = result.mmr || null;
            list.push({
                bbox,
                vehicleJpegB64: result.vehicleJpegB64,
                sharpness: result.sharpness != null
                    ? Number(result.sharpness)
                    : (vv.sharpness != null ? Number(vv.sharpness) : 0),
                label: vv.label || null,
                make: vv.make || (mmr0 && mmr0.make) || null,
                model: vv.model || (mmr0 && mmr0.model) || null,
                color: vv.color || (mmr0 && mmr0.color) || null,
                mmrText: vv.mmrText || (mmr0 && mmr0.mmrText) || null,
                mmr: mmr0,
            });
        }
    }
    return list.filter((e) => e.vehicleJpegB64);
}
...
function publishTick(tick) {
    ...
    tick.id = captureId;
    /* STRUCTURAL ISOLATION — OCR vs camera metadata (no string guessing) */
    tick.plate_text = (tick.plate_text != null && String(tick.plate_text).trim())
        ? String(tick.plate_text).trim()
        : ((tick.plateText != null && String(tick.plateText).trim())
            ? String(tick.plateText).trim()
            : ((tick.plate != null && String(tick.plate).trim()) ? String(tick.plate).trim() : null));
    tick.plateText = tick.plate_text;
    if (tick.plate_text && !tick.plate) tick.plate = tick.plate_text;
    tick.camera_id = String(tick.camera_id || tick.camId || '').trim() || null;
    if (tick.camera_id && !tick.camId) tick.camId = tick.camera_id;
    tick.device_name = String(tick.device_name || tick.deviceLabel || tick.bwcUser || '').trim() || null;
    tick.deviceLabel = tick.device_name || tick.deviceLabel || null;
    tick.macroCropUrl = tick.macroCropUrl || tick.vehicleUrl || null;
    tick.microCropUrl = tick.microCropUrl || tick.cropUrl || null;
    tick.bwcUser = tick.bwcUser || tick.device_name || tick.deviceLabel || null;
    tick.source = tick.source || 'live';
    if (tick.dwellSeconds == null) tick.dwellSeconds = 0;
    if (tick.isUpdate == null) tick.isUpdate = false;
    if (IS_CHILD || (deps && deps.emit)) {
        try {
        ...
        if (IS_CHILD) childSend({ type: 'emit', event: 'anpr-crop-tick', payload: tick, camId: tick.camId });
        else deps.emit('anpr-crop-tick', tick, tick.camId);
    }
    try {
    ...
    if (listMatch && listMatch.id) {
        ...
        if (now - prev >= HIT_DEDUPE_MS) {
            ...
            if (IS_CHILD) {
                childSend({ type: 'emit', event: 'anpr-list-hit', payload: hit, camId: tick.camId });
                childSend({ type: 'onHit', hit });
            } else {
...
/**
 * Text-based cooldown — discard exact same plate string within N ms (default 10s).
 * @returns {{ ok: boolean, reason?: string, ageMs?: number, key?: string }}
 */
function acceptPlateTextCooldown(tickOrText) {
    const key = typeof tickOrText === 'string'
        ? String(tickOrText).replace(/[\s\-_.]/g, '').toUpperCase()
        : plateDwellKey(tickOrText);
    if (!key || key === 'UNCLEAR') return { ok: false, reason: 'empty' };
    const now = Date.now();
    if (plateTextCooldown.size > 400) {
        for (const [k, ts] of plateTextCooldown.entries()) {
            if (now - ts > PLATE_TEXT_COOLDOWN_MS * 3) plateTextCooldown.delete(k);
        }
    }
    const prev = plateTextCooldown.get(key) || 0;
    const age = now - prev;
    if (prev && age < PLATE_TEXT_COOLDOWN_MS) {
        try {
            console.log('[ANPR-FUNNEL] plate_text_cooldown discard=' + key
                + ' ageMs=' + age + ' windowMs=' + PLATE_TEXT_COOLDOWN_MS);
        } catch (_) { /* ignore */ }
        return { ok: false, reason: 'cooldown', ageMs: age, key };
    }
    plateTextCooldown.set(key, now);
    return { ok: true, key };
}
        ...
        if (now - record.lastEmit > DWELL_UPDATE_MS) {
            ...
            if (IS_CHILD || (deps && deps.emit)) {
                try {
                    ...
                    if (IS_CHILD) childSend({ type: 'emit', event: 'anpr-crop-update', payload: upd, camId: tick.camId });
                    else deps.emit('anpr-crop-update', upd, tick.camId);
                } catch (_) { /* ignore */ }
            }
        }
        return 'dwell';
...
async function emitHarvestedAsync() {
    ...
    try {
        ...
        /* Multi-target concurrent /read-macro prefetch (all harvest jobs in parallel) */
        const ocrPrefetch = new Map();
        const prefetchJobs = harvested.filter((j) => j && !j.discard);
        await Promise.allSettled(prefetchJobs.map(async (job) => {
            const macroPaths = Array.isArray(job.macroPaths) && job.macroPaths.length
                ? job.macroPaths.slice()
                : (job.macroPath ? [job.macroPath] : []);
            const mp = macroPaths.slice(0, FLUSH_MACROS)[0];
            if (!mp) return;
            const key = String(job.camId || '') + '|' + String(job.trackId);
            const trackKey = job.trackId != null ? String(job.trackId) : null;
            try {
                ocrPrefetch.set(key, await anprPlateRead.readMacroPath(mp, { trackId: trackKey }));
                console.log(
                    '[ANPR-FUNNEL] OCR_sent_concurrent cam=' + String(job.camId || '')
                    + ' track=' + String(job.trackId || '')
                    + ' jobs=' + prefetchJobs.length
                );
            } catch (err) {
                ocrPrefetch.set(key, {
                    ok: false,
        ...
        for (let i = 0; i < harvested.length; i++) {
            ...
            let result = ocrPrefetch.get(String(job.camId || '') + '|' + String(job.trackId)) || null;
            const macroPaths = Array.isArray(job.macroPaths) && job.macroPaths.length
                ? job.macroPaths.slice()
                : (job.macroPath ? [job.macroPath] : []);
            ...
            if (ocrPaths.length) {
                console.log(
                    '[ANPR-FUNNEL] OCR_sent_after_trackBestFrame='
                    + ocrPaths.length
                    + ' cam=' + String(job.camId || '')
                    + ' track=' + String(job.trackId || '')
                    + ' harvestedJobs=' + harvested.length
                    + ' prefetched=' + (result ? '1' : '0')
                );
            }
            try {
                /* Dual-engine: prefer concurrent prefetch; else sequential */
                for (let mi = 0; mi < ocrPaths.length; mi++) {
                    const mp = ocrPaths[mi];
                    if (!mp) continue;
                    let one = (mi === 0 && result) ? result : null;
            ...
            if (plateOk && Number.isFinite(normalizedConf) && normalizedConf >= 95 && !alreadyFlushed) {
                ...
                if (!acceptPlateTextCooldown(tick).ok) {
                    try { anprTrackBestFrame.markFlushed(job.camId, job.trackId); } catch (_) { /* ignore */ }
                    continue;
                }
            ...
            if (alreadyFlushed && plateOk) {
                if (!acceptPlateTextCooldown(tick).ok) continue;
                if (gateDwellEmit(tick) === 'dwell') continue;
                continue;
            }
            ...
            if (tick.plateCompact || tick.plate || tick.plateText) {
                /* Post-OCR 10s text cooldown — discard identical plate strings */
                if (!acceptPlateTextCooldown(tick).ok) {
                    try { anprTrackBestFrame.markFlushed(job.camId, job.trackId); } catch (_) { /* ignore */ }
                    continue;
                }
...
async function grabCam(camId) {
    ...
    const t0 = Date.now();
    try {
        /* Pushed-FLV workers (child fork or external ingest) — no WVP memory here */
        if (usesPushedFlv()) {
            const flv = childLiveFlv[String(camId)];
            if (flv && typeof frLiveProbe.grabJpegFromFlv === 'function') {
                jpeg = await frLiveProbe.grabJpegFromFlv(flv, { camId: String(camId) });
            } else {
        ...
        } else if (usesPushedFlv()) {
            try {
                console.log('[anpr-ingest] grab skip', String(camId), String(err && err.message || err).slice(0, 120));
            } catch (_) { /* ignore */ }
        }
        return;
    }
...
async function producerTick() {
    ...
    try {
        ...
        const now = Date.now();
        /* No extra sleep gate — setInterval already paces grabs (was double-throttle → idle CPU). */
        lastProducerAt = now;
...
async function inferFrame(camId, jpeg, grabbedAt) {
    ...
    const entries = vehicleEntriesFromTrackResult(result);
    if (!entries.length) return;

    let anyShoot = false;
    for (let i = 0; i < entries.length; i++) {
        const ent = entries[i];
        if (!ent || !ent.bbox || !ent.vehicleJpegB64) continue;
        const obs = anprTrackBestFrame.observe(camId, ent.bbox, {
            sharpness: ent.sharpness || 0,
            vehicleJpegB64: ent.vehicleJpegB64,
            vehicleLabel: ent.label ? String(ent.label) : null,
            frameUuid: result.frameUuid || null,
            deviceLabel: deviceLabel(camId),
            at: new Date().toISOString(),
            make: ent.make || null,
            model: ent.model || null,
            color: ent.color || null,
            mmrText: ent.mmrText || null,
            mmr: ent.mmr || null,
        });
        if (obs && obs.shootFirst) anyShoot = true;
    }
    /* Multi-target shoot-first: one harvest kick fires all ready tracks concurrently */
    if (anyShoot) {
