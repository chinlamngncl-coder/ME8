...
const MAX_DURATION_SEC = Math.max(10, parseInt(process.env.FM_FR_OFFLINE_MAX_SEC || '3600', 10) || 3600);
const SAMPLE_FPS = Math.max(0.25, Math.min(2, parseFloat(process.env.FM_FR_OFFLINE_FPS || '1') || 1));
const MAX_FRAMES = Math.max(5, parseInt(process.env.FM_FR_OFFLINE_MAX_FRAMES || '100000', 10) || 100000);
...
function saveCropB64(b64, meta) {
    ...
    try {
        ...
        /* MASTER-CONSOLIDATION-PATCH-V1 — permanent evidence archive, no more 120-file wipe */
        try {
            const m = meta || {};
            analyticsCaptureStore.saveJpeg('fr', 'micro', raw, {
                username: m.username || 'system',
                bwcName: m.deviceLabel || m.camId || 'offline-video',
                camId: m.camId || '',
