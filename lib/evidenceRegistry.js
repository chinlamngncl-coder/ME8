...
const evidenceDeleteQueue = require('./evidenceDeleteQueue');
const evidenceGpsLock = require('./evidenceGpsLock');
...
function isQueuedDeleteFile(file) {
    return !!(file && String(file.storageTier || '') === 'queued_delete');
}
...
function decorateFileRow(file) {
    ...
    return Object.assign({}, file, {
        ...
        sortType: String(file.sortType || '').trim(),
    });
}
...
async function registerFromUpload(opts) {
    ...
    let id = existing ? existing.id : newFileId();
    const now = new Date().toISOString();
    const upsertRow = (rowId) => ({
        id: rowId,
    ...
    try {
        await siteDb.upsertEvidenceFile(upsertRow(id));
    } catch (err) {
        /* DB-UNIQUENESS-AND-LOCKS-V1 — UNIQUE(relative_path) (migration 032): a concurrent
           register for the same file won the insert. Adopt its id and update that row. */
        if (!err || err.code !== '23505') throw err;
        const winner = await siteDb.findEvidenceByRelative(rel);
        if (!winner) throw err;
        existing = winner;
        id = winner.id;
        await siteDb.upsertEvidenceFile(upsertRow(id));
    ...
    try {
        await evidenceGpsLock.bindOnCatalog({
            id: id,
            deviceId: opts.deviceId || (existing && existing.device_id) || null,
            uploadedAt: (existing && existing.uploaded_at) || now,
            relativePath: rel,
            fileName: opts.originalFileName || opts.fileName || rel,
            fullPath: onDisk.fullPath,
        });
    } catch (_) { /* catalog admit must not fail if GPS lock is empty */ }
    deferRepairFile(id);
    return id;
}
...
async function scanFtpRoot(limit, onAdmitted) {
    ...
    for (const item of files) {
        ...
        try {
            ...
            const deviceIdGuess = (() => {
                try {
                    return require('./dockFtpIngestWatch').parseDeviceIdFromRel(item.rel) || null;
                } catch (_) {
                    return null;
                }
            })();
...
/** EVIDENCE-DELETE-QUEUE-7D-V1 — soft-delete into 7-day queue (file stays on disk). */
async function queueDelete(fileId, actor) {
    const file = await siteDb.getEvidenceFile(fileId);
    if (!file) throw new Error('Evidence file not found');
    if (isQueuedDeleteFile(file)) {
        const existing = evidenceDeleteQueue.get(fileId);
        const row = decorateFileRow(file);
        return Object.assign({}, row, {
            archived: false,
            queuedDelete: true,
            deleteQueue: existing,
        });
    }
    if (isArchivedFile(file)) {
        throw new Error('Restore from archive before queuing delete');
    }
    const previousTier = String(file.storageTier || 'local');
    const actorName = actor && (actor.username || actor.displayName) ? (actor.username || actor.displayName) : null;
    const item = evidenceDeleteQueue.buildQueueItem(file, actorName, previousTier);
    await siteDb.upsertEvidenceFile(Object.assign({}, file, {
        storageTier: 'queued_delete',
        syncStatus: file.syncStatus || 'synced',
    }));
    evidenceDeleteQueue.upsertItem(item);
    const out = decorateFileRow(await siteDb.getEvidenceFile(fileId));
    return Object.assign({}, out, { archived: false, queuedDelete: true, deleteQueue: item });
}

function isRetentionHold(meta) {
    if (!meta) return false;
    if (meta.sosIncidentId) return true;
    const tags = Array.isArray(meta.tags) ? meta.tags : [];
    for (let i = 0; i < tags.length; i++) {
        /* retention-hold / case hold / keep forever — never auto-purge */
...
/**
 * EVIDENCE-RETENTION-ENFORCE-V1 — age > evidence.retentionDays → 7-day delete queue.
 * retentionDays 0 = until manual (no auto-queue).
 */
async function enqueueRetentionDue(settings) {
    const platformLimits = require('./platformLimits');
    const lim = platformLimits.loadLimits();
    const licenseDays = lim.evidenceRetentionDays;
    const days = licenseDays != null && Number.isFinite(licenseDays)
        ? Math.max(0, licenseDays)
        : Math.max(0, parseInt(settings && settings.evidence && settings.evidence.retentionDays, 10) || 0);
    if (!days) {
        return { ok: true, queued: 0, skipped: 0, failed: 0, reason: 'retention_disabled' };
    }
    if (!siteDb.isReady()) {
        return { ok: false, queued: 0, skipped: 0, failed: 0, error: 'catalog_unavailable' };
    }
    const cutoffIso = new Date(Date.now() - days * 86400000).toISOString();
    const files = await siteDb.listActiveEvidenceUploadedBefore(cutoffIso, 200);
    const actor = { username: 'system-retention' };
    let queued = 0;
    let skipped = 0;
    let failed = 0;
    for (let i = 0; i < files.length; i++) {
        ...
        if (isQueuedDeleteFile(file) || isArchivedFile(file)) {
            skipped += 1;
            continue;
        }
        try {
            const meta = await siteDb.getEvidenceMeta(file.id);
            if (isRetentionHold(meta)) {
                skipped += 1;
                continue;
            }
            try {
                const caseIds = await siteDb.listCaseFileIdsByEvidence(file.id);
            ...
            await queueDelete(file.id, actor);
            queued += 1;
        } catch (_) {
            failed += 1;
        }
    }
    return { ok: true, queued: queued, skipped: skipped, failed: failed, retentionDays: days, cutoffIso: cutoffIso };
}

/** Restore from delete queue back to Library (active). */
async function restoreFromDeleteQueue(fileId, actor) {
    const file = await siteDb.getEvidenceFile(fileId);
    if (!file) throw new Error('Evidence file not found');
    if (!isQueuedDeleteFile(file)) throw new Error('Evidence file is not queued for deletion');
    const item = evidenceDeleteQueue.get(fileId);
    const tier = (item && item.previousTier && item.previousTier !== 'queued_delete')
        ? item.previousTier
        : 'local';
    await siteDb.upsertEvidenceFile(Object.assign({}, file, {
        storageTier: tier === 'archived' ? 'local' : tier,
        syncStatus: file.syncStatus || 'synced',
    }));
    evidenceDeleteQueue.remove(fileId);
    const prev = await siteDb.getEvidenceMeta(fileId) || { evidenceFileId: fileId, tags: [] };
    await siteDb.upsertEvidenceMeta(Object.assign({}, prev, {
        evidenceFileId: fileId,
        notes: prev.notes || '',
    ...
    const out = decorateFileRow(await siteDb.getEvidenceFile(fileId));
    return Object.assign({}, out, { archived: isArchivedFile(out), queuedDelete: false });
}

/** Purge due queued deletes (disk + catalog). */
async function purgeDueQueuedDeletes() {
    const due = evidenceDeleteQueue.dueItems();
    const results = [];
    for (let i = 0; i < due.length; i++) {
        const item = due[i];
        const id = item && item.evidenceFileId;
        if (!id) continue;
        try {
            const file = await siteDb.getEvidenceFile(id);
            let fullPath = null;
            if (file) fullPath = resolveFilePath(file);
            if (fullPath && fs.existsSync(fullPath)) {
                try { fs.unlinkSync(fullPath); } catch (_) { /* continue DB purge */ }
            }
            if (file) await siteDb.deleteEvidenceFileCascade(id);
            evidenceDeleteQueue.remove(id);
            results.push({ evidenceFileId: id, ok: true });
        } catch (err) {
            results.push({
                evidenceFileId: id,
                ok: false,
    ...
    return { purged: results.filter(function (r) { return r.ok; }).length, results: results };
}

/** Immediate disk+catalog remove for disk-pressure FIFO. Caller must check protection first. */
async function purgeFileNow(id) {
    ...
    if (file) fullPath = resolveFilePath(file);
    if (fullPath && fs.existsSync(fullPath)) {
        try { fs.unlinkSync(fullPath); } catch (_) { /* catalog still drops */ }
    }
    if (file) await siteDb.deleteEvidenceFileCascade(fileId);
    try { evidenceDeleteQueue.remove(fileId); } catch (_) { /* ignore */ }
    return { ok: true };
}

function listDeleteQueue() {
    return evidenceDeleteQueue.list();
}
...
module.exports = {
    ...
    restoreFile,
    queueDelete,
    restoreFromDeleteQueue,
    purgeDueQueuedDeletes,
    purgeFileNow,
    enqueueRetentionDue,
    listDeleteQueue,
    isArchivedFile,
    ...
    getFile: async (id) => {
        const file = await siteDb.getEvidenceFile(id);
        if (!file) return null;
        const queued = isQueuedDeleteFile(file);
        return Object.assign({}, decorateFileRow(file), {
            archived: isArchivedFile(file),
            queuedDelete: queued,
            deleteQueue: queued ? evidenceDeleteQueue.get(id) : null,
        });
    },
