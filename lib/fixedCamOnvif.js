...
/**
 * Light network probe for Discovery Scanner — auth + device info only.
 * Does NOT call GetProfiles or clock sync (keeps CIDR scans fast).
 */
async function lightNetworkProbe(opts) {
    const host = cameraHost(opts && opts.host);
    const port = parseInt(opts && opts.port, 10) || 80;
    const user = String((opts && opts.user) || '');
    const password = String((opts && opts.password) || '');
    const path = String((opts && opts.devicePath) || '/onvif/device_service');
    const timeout = Math.min(8000, Math.max(2000, parseInt(opts && opts.timeoutMs, 10) || 5000));
    let client = null;
    try {
        ...
        const info = await new Promise(function (resolve) {
            if (!client || typeof client.getDeviceInformation !== 'function') {
                return resolve(null);
            }
            client.getDeviceInformation(function (err, data) {
                resolve(err ? null : (data || null));
            });
        });
        let mac = '';
        try {
            mac = await new Promise(function (resolve) {
                if (!client || typeof client.getNetworkInterfaces !== 'function') {
                    return resolve('');
                }
                client.getNetworkInterfaces(function (err, ifaces) {
                    if (err || !ifaces) return resolve('');
                    const list = Array.isArray(ifaces) ? ifaces : [ifaces];
                    for (let i = 0; i < list.length; i += 1) {
                        const row = list[i] || {};
                        const infoRow = row.info || row.Info || row;
                        const hw = infoRow.hwAddress || infoRow.HwAddress
                            || infoRow.hardwareAddress || infoRow.MAC
                            || infoRow.mac || '';
                        if (hw) return resolve(String(hw));
                    }
                    resolve('');
                });
            });
        ...
        const manufacturer = info
            ? String(info.manufacturer || info.Manufacturer || '')
            : '';
        const model = info ? String(info.model || info.Model || '') : '';
        const serial = info
            ? String(info.serialNumber || info.SerialNumber || '')
            : '';
        ...
        const status = /auth|password|401|403|Unauthorized|NotAuthorized/i.test(msg)
            ? 'Auth_Failed'
            : 'Timeout';
        return {
            ok: false,
