...
function clearSosCapturePack(alarmCamId) {
    ...
    if (!alarmId) {
        sosPackByCam.clear();
        [...sosSegmentState.keys()].forEach(cancelSosSegmentRestart);
        return;
    }
    [...sosPackByCam.entries()].forEach(([cam, meta]) => {
        if (meta && meta.alarmCamId === alarmId) {
            sosPackByCam.delete(cam);
            cancelSosSegmentRestart(cam);
        }
    });
}
...
/* ── SOS-OPEN-INCIDENT-RECORD-CONTINUITY-V1 ─────────────────────────────────
 * Open incident ⇒ HQ capture must be running. When the WVP ffmpeg child dies
 * (BWC toggled SOS off, Wi-Fi drop, ZLM hiccup) finalize that file as segment N
 * and restart segment N+1 on the same incident. Only HQ ack ends the chain.
 * Intentional stops (killWvpChild nulls rec.child) are not restarted.
 */
const SEG_MAX_PER_INCIDENT = 10;
const SEG_RETRY_MAX_ATTEMPTS = 40;
const SEG_RETRY_BASE_MS = 2000;
const SEG_RETRY_MAX_MS = 15000;
/** camId → { incidentId, segments, attempt, timer } */
const sosSegmentState = new Map();

/** Strict: only a truly open (not acknowledged) alarm — alarm cam or response pack. */
function openIncidentIdForCam(id) {
    if (!sosIncidentsMod || typeof sosIncidentsMod.getOpenAlarms !== 'function') return null;
    const open = sosIncidentsMod.getOpenAlarms() || [];
    const hit = open.find((e) => e && (String(e.cameraId) === id
        || (Array.isArray(e.responseCamIds) && e.responseCamIds.map(String).includes(id))));
    return hit ? hit.id : null;
}

function cancelSosSegmentRestart(id) {
    const st = sosSegmentState.get(id);
    if (st && st.timer) clearTimeout(st.timer);
    sosSegmentState.delete(id);
}

function scheduleSosSegmentRestart(id, incidentId) {
    let st = sosSegmentState.get(id);
    if (!st || st.incidentId !== incidentId) {
        st = { incidentId, segments: 1, attempt: 0, timer: null };
        sosSegmentState.set(id, st);
    }
    if (st.segments >= SEG_MAX_PER_INCIDENT || st.attempt >= SEG_RETRY_MAX_ATTEMPTS) {
        log.web.warn('live capture segment restart stopped (cap)', {
            camId: id, incidentId, segments: st.segments, attempt: st.attempt,
            path: 'sos-open-incident-record-continuity-v1',
        });
        return;
    }
    const delay = Math.min(SEG_RETRY_MAX_MS, SEG_RETRY_BASE_MS * Math.pow(2, st.attempt));
    st.attempt += 1;
    if (st.timer) clearTimeout(st.timer);
    st.timer = setTimeout(function () {
        st.timer = null;
        if (active.has(id)) { st.attempt = 0; return; } /* dashboard play path already restarted */
        if (!openIncidentIdForCam(id)) { sosSegmentState.delete(id); return; }
        if (!((settings().evidence || {}).liveCaptureEnabled)) { sosSegmentState.delete(id); return; }
        try {
            startForSos(id);
            log.web.info('live capture segment restarted', {
                camId: id, incidentId, segment: st.segments + 1,
                path: 'sos-open-incident-record-continuity-v1',
            });
        } catch (err) {
            /* upstream not back yet (BWC not streaming) → back off and try again */
            log.web.info('live capture segment restart deferred', {
                camId: id, attempt: st.attempt,
                message: err && err.message ? err.message : String(err),
            });
            scheduleSosSegmentRestart(id, incidentId);
        }
    }, delay);
}

function onWvpChildDied(id, rec, code) {
    if (active.get(id) !== rec) return;
    active.delete(id);
    const incidentId = rec.sosAuto ? openIncidentIdForCam(id) : null;
    let hasData = false;
    try { hasData = fs.existsSync(rec.full) && fs.statSync(rec.full).size > 64; } catch (_) { /* ignore */ }
    log.web.warn('live capture ffmpeg ended unexpectedly', {
        camId: id, code, hasData, incidentOpen: !!incidentId, fileName: rec.fileName,
        path: 'sos-open-incident-record-continuity-v1',
    });
    if (hasData) {
        const st = sosSegmentState.get(id);
        if (st && st.incidentId === incidentId) { st.segments += 1; st.attempt = 0; }
        Promise.resolve()
        ...
        try { fs.unlinkSync(rec.full); } catch (_) { /* ignore */ }
    }
    if (!incidentId) { cancelSosSegmentRestart(id); return; }
    scheduleSosSegmentRestart(id, incidentId);
}
...
/* LC-SEGMENT-END-FROM-MEDIA-V1 — real media duration (seconds) via ffmpeg header read,
   before encryption. null on any failure → caller falls back to wall clock. */
function probeMediaDurationSec(fullPath) {
    return new Promise(function (resolve) {
        let child;
        try {
            child = spawn(resolveFfmpegPath(), ['-hide_banner', '-i', fullPath], {
                windowsHide: true, stdio: ['ignore', 'ignore', 'pipe'],
            });
        } catch (_) { resolve(null); return; }
        let err = '';
        const timer = setTimeout(function () { try { child.kill('SIGKILL'); } catch (_) { /* ignore */ } }, 8000);
        child.stderr.on('data', function (b) { if (err.length < 65536) err += String(b || ''); });
        ...
        child.on('exit', function () {
            ...
            const sec = (+m[1]) * 3600 + (+m[2]) * 60 + (+m[3]) + frac;
...
async function registerCapture(camId, rec) {
    ...
    /* LC-SEGMENT-END-FROM-MEDIA-V1 — segment end = start + media length (was finalize wall time →
       phantom blue tail on the Investigation timeline when ffmpeg died early). */
    let endAtIso = new Date().toISOString();
    try {
        const durSec = await probeMediaDurationSec(rec.full);
        const startMs = Date.parse(rec.startedAt);
        if (durSec && Number.isFinite(startMs)) {
            const mediaEnd = startMs + Math.round(durSec * 1000);
            if (mediaEnd < Date.parse(endAtIso)) endAtIso = new Date(mediaEnd).toISOString();
            rec.mediaDurationSec = durSec;
        }
    } catch (_) { /* fall back to wall clock */ }
    ...
    log.web.info('live capture registered', {
        camId, evidenceId, fileName: rec.fileName,
        mediaDurationSec: rec.mediaDurationSec || null, endAt: endAtIso,
    });
    ...
    try {
        ...
        await sosEvidenceIndexer.indexLiveCaptureFile(camId, rec.full, {
            ...
            endAt: endAtIso, /* LC-SEGMENT-END-FROM-MEDIA-V1 */
            evidenceId,
        });
    } catch (err) {
...
function startWvpFromFlv(camId, flvUrl) {
    ...
    active.set(id, rec);
    /* SOS-OPEN-INCIDENT-RECORD-CONTINUITY-V1 — unexpected exit → finalize segment, restart while incident open */
    child.on('exit', function (code) {
        if (rec.child !== child) return; /* intentional stop via killWvpChild */
        rec.child = null;
        try { onWvpChildDied(id, rec, code); } catch (_) { /* never throw into ffmpeg event */ }
    });
...
async function stop(camId) {
    ...
    active.delete(id);
    cancelSosSegmentRestart(id); /* operator stop is intentional — no auto restart */
...
function onStreamStopped(camId) {
    ...
    if (!rec) return;
    active.delete(id);
    cancelSosSegmentRestart(id); /* stream torn down on purpose (stop-live / hard-stop) */
    if (rec.mode === 'pool_tee') {
        try { liveStreamPool.setLiveCaptureRecording(id, null); } catch (_) { /* ignore */ }
    }
    Promise.resolve()
