...
function init(dir) {
    ...
    `);
    try {
        const cols = db.prepare('PRAGMA table_info(evidence_meta)').all().map(function (c) { return c.name; });
        if (cols.indexOf('tags_json') < 0) {
            db.exec('ALTER TABLE evidence_meta ADD COLUMN tags_json TEXT');
        }
    } catch (_) { /* ignore */ }
    return db;
}
...
/**
 * Paginated evidence catalog.
 * opts.status: 'active' (default) | 'archived' | 'all'
 * Optional tagContains (lowercase substring match on tags_json).
 */
function listEvidenceFilesPage(opts) {
    if (!db) return { files: [], total: 0, page: 1, pageSize: 50, pageCount: 1 };
    const pageSize = Math.min(100, Math.max(10, parseInt(opts && opts.pageSize, 10) || 50));
    let page = Math.max(1, parseInt(opts && opts.page, 10) || 1);
    const tag = opts && opts.tag ? String(opts.tag).trim().toLowerCase() : '';
    const statusRaw = opts && opts.status != null ? String(opts.status).trim().toLowerCase() : 'active';
    const status = (statusRaw === 'archived' || statusRaw === 'all') ? statusRaw : 'active';
    let tierSql = '';
    if (status === 'archived') {
        tierSql = " AND COALESCE(ef.storage_tier, 'local') = 'archived' ";
    ...
    if (tag) {
        ...
        total = db.prepare(`
            SELECT COUNT(*) AS c FROM evidence_files ef
            INNER JOIN evidence_meta em ON em.evidence_file_id = ef.id
            WHERE em.tags_json IS NOT NULL AND lower(em.tags_json) LIKE ?
            ${tierSql.replace(/ef\./g, 'ef.')}
        `.replace(' AND COALESCE', ' AND COALESCE')).get(like).c;
        // rebuild count with aliases consistent
        total = db.prepare(`
            SELECT COUNT(*) AS c FROM evidence_files ef
            INNER JOIN evidence_meta em ON em.evidence_file_id = ef.id
            WHERE em.tags_json IS NOT NULL AND lower(em.tags_json) LIKE ?
            ${status === 'archived'
                ? "AND COALESCE(ef.storage_tier, 'local') = 'archived'"
                : (status === 'active' ? "AND COALESCE(ef.storage_tier, 'local') != 'archived'" : '')}
        `).get(like).c;
        const pageCount = Math.max(1, Math.ceil(total / pageSize));
        if (page > pageCount) page = pageCount;
        const offset = (page - 1) * pageSize;
        rows = db.prepare(`
            SELECT ef.id, ef.source, ef.storage_tier, ef.relative_path, ef.file_name, ef.byte_size,
                   ef.device_id, ef.operator_name, ef.uploaded_at, ef.peer_ip, ef.sync_status
            ...
            ${status === 'archived'
                ? "AND COALESCE(ef.storage_tier, 'local') = 'archived'"
                : (status === 'active' ? "AND COALESCE(ef.storage_tier, 'local') != 'archived'" : '')}
            ORDER BY ef.uploaded_at DESC LIMIT ? OFFSET ?
        `).all(like, pageSize, offset);
        return {
            files: rows.map(mapEvidenceFileRow),
            total: total,
            page: page,
            pageSize: pageSize,
            pageCount: pageCount,
    ...
    if (status === 'archived') {
        total = db.prepare(`SELECT COUNT(*) AS c FROM evidence_files WHERE COALESCE(storage_tier, 'local') = 'archived'`).get().c;
    } else if (status === 'active') {
        total = db.prepare(`SELECT COUNT(*) AS c FROM evidence_files WHERE COALESCE(storage_tier, 'local') != 'archived'`).get().c;
    } else {
        total = db.prepare('SELECT COUNT(*) AS c FROM evidence_files').get().c;
    }
    const pageCount = Math.max(1, Math.ceil(total / pageSize));
    if (page > pageCount) page = pageCount;
    const offset = (page - 1) * pageSize;
    if (status === 'archived') {
        rows = db.prepare(`
            SELECT id, source, storage_tier, relative_path, file_name, byte_size,
                   device_id, operator_name, uploaded_at, peer_ip, sync_status
            FROM evidence_files WHERE COALESCE(storage_tier, 'local') = 'archived'
        ...
        rows = db.prepare(`
            SELECT id, source, storage_tier, relative_path, file_name, byte_size,
                   device_id, operator_name, uploaded_at, peer_ip, sync_status
            FROM evidence_files ORDER BY uploaded_at DESC LIMIT ? OFFSET ?
        `).all(pageSize, offset);
    }
    return {
        files: rows.map(mapEvidenceFileRow),
...
function upsertEvidenceMeta(row) {
    ...
    const now = new Date().toISOString();
    const tagsJson = Array.isArray(row.tags) ? JSON.stringify(row.tags) : (row.tagsJson != null ? String(row.tagsJson) : '[]');
    db.prepare(`
        INSERT INTO evidence_meta (
            evidence_file_id, notes, sos_incident_id, trim_start_sec, trim_end_sec,
            dock_id, dock_bay, tags_json, updated_at, updated_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
/** Map evidence_file_id → tags[] for catalog enrichment (files with empty tags omitted). */
function listEvidenceTagsByFileId(limit) {
    if (!db) return {};
...
/**
 * mob-evidence-redacted-exports-browser-v1 — paginated redact exports + source file fields.
 * status: finalized | pending | all
 */
function listRedactedEvidenceExportsPage(opts) {
    if (!db) {
        return { exports: [], total: 0, page: 1, pageSize: 50, pageCount: 0 };
    }
    opts = opts || {};
    const pageSize = Math.min(100, Math.max(1, parseInt(opts.pageSize, 10) || 50));
    let page = Math.max(1, parseInt(opts.page, 10) || 1);
    const statusRaw = String(opts.status || 'finalized').trim().toLowerCase();
    const status = (statusRaw === 'pending' || statusRaw === 'all') ? statusRaw : 'finalized';
    const q = String(opts.q || '').trim().toLowerCase();
    ...
    if (status === 'finalized') {
        where.push("json_extract(e.meta_json, '$.status') = 'finalized'");
    } else if (status === 'pending') {
        where.push("(json_extract(e.meta_json, '$.status') IS NULL OR json_extract(e.meta_json, '$.status') != 'finalized')");
    }

    if (from) {
        where.push("COALESCE(json_extract(e.meta_json, '$.finalizedAt'), e.created_at) >= ?");
        params.push(from);
    }
    if (to) {
        where.push("COALESCE(json_extract(e.meta_json, '$.finalizedAt'), e.created_at) <= ?");
    ...
    if (q) {
        ...
        where.push(`(
            lower(e.file_name) LIKE ? OR lower(e.export_id) LIKE ?
            OR lower(IFNULL(f.file_name,'')) LIKE ?
            OR lower(IFNULL(f.operator_name,'')) LIKE ?
            OR lower(IFNULL(f.device_id,'')) LIKE ?
            OR lower(IFNULL(e.actor,'')) LIKE ?
        )`);
        params.push(like, like, like, like, like, like);
    }

    if (tag) {
        where.push("lower(IFNULL(m.tags_json,'')) LIKE ?");
    ...
    const fromSql = `
        ...
        LEFT JOIN evidence_files f ON f.id = e.evidence_file_id
        LEFT JOIN evidence_meta m ON m.evidence_file_id = e.evidence_file_id
        WHERE ${whereSql}
    `;

    const total = db.prepare('SELECT COUNT(*) AS n ' + fromSql).get(...params).n || 0;
    const pageCount = total ? Math.ceil(total / pageSize) : 0;
    if (pageCount && page > pageCount) page = pageCount;
    const offset = (page - 1) * pageSize;

    const rows = db.prepare(`
        SELECT e.export_id, e.evidence_file_id, e.export_type, e.file_name, e.byte_size,
               e.created_at, e.actor, e.meta_json,
               f.file_name AS source_file_name,
               f.device_id AS source_device_id,
               f.operator_name AS source_operator_name,
               f.uploaded_at AS source_uploaded_at,
               m.tags_json AS source_tags_json
        ${fromSql}
        ORDER BY COALESCE(json_extract(e.meta_json, '$.finalizedAt'), e.created_at) DESC
        LIMIT ? OFFSET ?
    `).all(...params, pageSize, offset);

    const exports = rows.map(function (row) {
        ...
        return Object.assign({}, base, {
            ...
            sourceOfficer: row.source_operator_name || null,
            sourceUploadedAt: row.source_uploaded_at || null,
            sourceTags: parseTagsJson(row.source_tags_json),
...
function deleteEvidenceExport(exportId) {
    if (!db || !exportId) return false;
    const info = db.prepare('DELETE FROM evidence_exports WHERE export_id = ?').run(String(exportId));
    return info.changes > 0;
}
...
module.exports = {
    ...
    listEvidenceExports,
    listRedactedEvidenceExportsPage,
    getEvidenceExport,
    deleteEvidenceExport,
