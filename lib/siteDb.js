    ...
    initPromise = (async () => {
        ...
        try {
            ...
            try {
                ...
                if (!check.rows[0] || Number(check.rows[0].schema_version) < 9) {
                    throw new Error('PostgreSQL catalog schema migration did not complete (need version >= 9 analytics_capture_audit)');
                }
            } finally {
                client.release();
            }
