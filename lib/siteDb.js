    ...
    initPromise = (async () => {
        ...
        try {
            ...
            try {
                ...
                if (!check.rows[0] || Number(check.rows[0].schema_version) < 6) {
                    throw new Error('PostgreSQL catalog schema migration did not complete (need version >= 6 tactical_blueprint_placement)');
                }
            } finally {
                client.release();
            }
