...
function resolveCompiledExe() {
    const name = process.platform === 'win32' ? 'fr-engine.exe' : 'fr-engine';
...
function startProcess() {
    ...
    starting = new Promise((resolve) => {
        ...
        const py = compiled ? compiled : pythonBin(cfg.dir);
        const args = compiled
            ? []
            : (py === 'py'
        ...
        appendSidecarLog('\n--- ' + cfg.logTag + ' spawn ' + new Date().toISOString() + ' ---\n'
            + 'engine=' + cfg.engine + (compiled ? ' exe=' + compiled : ' python=' + py) + ' cwd=' + (compiled ? path.dirname(compiled) : cfg.dir) + ' port=' + cfg.port + '\n');
        if (!compiled && !fs.existsSync(cfg.dir)) {
            starting = null;
            resolve({
                ok: false,
        ...
        if (!compiled && !fs.existsSync(py) && py !== 'py' && py !== 'python3') {
            starting = null;
            resolve({
                ok: false,
        ...
        try {
            ...
            child = spawn(py, args, {
                cwd: compiled ? path.dirname(compiled) : cfg.dir,
                windowsHide: true,
                env: env,
...
async function bootstrap(opts) {
    ...
    if (bootstrapDone && !force) return health();
    bootstrapDone = true;
    const h = await health();
    if (h.ok) return h;
