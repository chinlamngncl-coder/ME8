...
class MessageReassembler {
    ...
    /**
     * Drop incomplete assemblies that have not received a chunk within maxAgeMs.
     * @param {number} [maxAgeMs=60000]
     * @returns {number} number of states removed
     */
    pruneStaleBuffers(maxAgeMs = 60000) {
        const maxAge = Number(maxAgeMs);
        const cutoff = Date.now() - (Number.isFinite(maxAge) && maxAge > 0 ? maxAge : 60000);
        let pruned = 0;
        for (const [key, state] of this._parts) {
            const touched = state && Number(state.lastTouched);
    ...
    ingest(header, msg) {
        ...
        if (!state) {
            ...
            state = { totalLen: msg.totalLen, chunks: new Map(), received: 0, meta: msg, lastTouched: now };
