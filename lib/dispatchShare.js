...
/* AIRGAP-OUTBOUND-LINKS-V1 — geo: URI opens the officer's on-device map app (no public host,
   no server-side outbound). Text exports carry plain coordinates via appendShareToDispatchText. */
...
function appendShareToDispatchText(text, shareUrl, lat, lon) {
    ...
    const hasPos = lat != null && lon != null && !Number.isNaN(Number(lat)) && !Number.isNaN(Number(lon));
    if (shareUrl) lines.push('', `Mobile map (full screen): ${shareUrl}`);
    if (hasPos) lines.push(`Location: ${Number(lat).toFixed(6)}, ${Number(lon).toFixed(6)}`);
