...
/** MOB-APPLY-PTT-CAMID-WVP-LAN-MAP-V1 — optional hook when REGISTER peer is learned. */
let onRegisterPeerFn = null;

function setOnRegisterPeer(fn) {
    onRegisterPeerFn = typeof fn === 'function' ? fn : null;
}
...
/* JSON-STORE-SINGLE-WRITER-V1 — cached map + atomic temp/rename write. A torn file or a
   read that races a write can no longer come back as {} and wipe every registered peer. */
const mapStore = require('./jsonStoreWriter').createStore({
    path: MAP_PATH,
    indent: 0,
    parse: (j) => (j && typeof j === 'object'
        ? { updatedAt: j.updatedAt || null, devices: (j.devices && typeof j.devices === 'object') ? j.devices : {} }
        : null),
    fallback: () => ({ updatedAt: null, devices: {} }),
});

function writeMap(mapObj) {
    const payload = Object.assign({}, mapObj || {}, { updatedAt: new Date().toISOString() });
    if (!payload.devices || typeof payload.devices !== 'object') payload.devices = {};
    mapStore.write(payload);
}

function readMap() {
    return mapStore.read();
}
...
function rememberRegisterPeer(deviceId, ip, port) {
    ...
    writeMap(map);
    if (onRegisterPeerFn) {
        try { onRegisterPeerFn(did, String(ip).trim()); } catch (_) { /* ignore */ }
    }
    return true;
}
...
/** PTT TCP peer IP → GB device id (WVP REGISTER peer rows only). */
function resolveGbIdByPeerIp(peerIp) {
    const ip = String(peerIp || '').trim().replace(/^::ffff:/i, '');
    if (!isLanIpv4(ip)) return null;
    const map = readMap();
    const devices = map.devices || {};
    for (const did of Object.keys(devices)) {
        if (!isBwcGbId(did)) continue;
        const hit = devices[did];
        const peer = (hit && (hit.peerIp || hit.ip)) ? String(hit.peerIp || hit.ip).trim() : '';
        if (peer === ip) return did;
    }
    return null;
}

/** Boot seed for camIdByIp — one row per WVP register peer. */
function eachRegisterPeerIp() {
    const out = [];
    const map = readMap();
    Object.keys(map.devices || {}).forEach((did) => {
        if (!isBwcGbId(did)) return;
        const hit = map.devices[did];
        const ip = hit && (hit.peerIp || hit.ip) ? String(hit.peerIp || hit.ip).trim() : '';
        if (isLanIpv4(ip)) out.push({ camId: did, ip: ip });
    });
    return out;
}
