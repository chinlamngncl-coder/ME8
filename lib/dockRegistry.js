...
function normalizeDock(input, existing) {
    ...
    if (patch.cloudKey != null) base.cloudKey = String(patch.cloudKey).trim();
    if (patch.lastSeenAt != null) base.lastSeenAt = String(patch.lastSeenAt);
    if (patch.localPort != null) base.localPort = String(patch.localPort).trim();
    if (patch.firmwareVersion != null) base.firmwareVersion = String(patch.firmwareVersion).trim();
    if (patch.priorityPort != null) base.priorityPort = String(patch.priorityPort).trim();
    if (patch.hddUsedPct != null && patch.hddUsedPct !== '') {
        const n = Number(patch.hddUsedPct);
        base.hddUsedPct = Number.isFinite(n) ? Math.max(0, Math.min(100, Math.round(n))) : null;
    }
...
function findByCloudKey(cloudKey) {
    const key = String(cloudKey || '').trim();
    if (!key) return null;
    return listDocks().find(function (d) {
        return String(d.cloudKey || '') === key || String(d.branchCode || '') === key;
    }) || null;
}

function upsertByCloudKey(input) {
    const key = String((input && input.cloudKey) || '').trim();
    if (!key) throw new Error('cloudKey required');
    const now = new Date().toISOString();
    const store = readStore();
    const idx = store.docks.findIndex(function (d) {
        return String(d.cloudKey || '') === key || String(d.branchCode || '') === key;
    });
    const patch = {
        cloudKey: key,
        branchCode: key,
        displayName: (input && input.displayName) || key,
        contactName: input && input.contactName,
        contactPhone: input && input.contactPhone,
        notes: input && input.notes,
        localPort: input && input.localPort,
        firmwareVersion: input && input.version,
        status: 'online',
        linkState: 'online',
        lastSeenAt: now,
    };
    if (idx < 0) {
        const dock = normalizeDock(patch, null);
        store.docks.push(dock);
        writeStore(store);
        return dock;
    }
    store.docks[idx] = normalizeDock(patch, store.docks[idx]);
    writeStore(store);
    return store.docks[idx];
}

function touchByCloudKey(cloudKey, extra) {
    const key = String(cloudKey || '').trim();
    if (!key) throw new Error('cloudKey required');
    const existing = findByCloudKey(key);
    if (!existing) throw new Error('Dock not found');
    return updateDock(existing.id, Object.assign({
        status: 'online',
        linkState: 'online',
