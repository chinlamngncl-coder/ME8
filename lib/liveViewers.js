...
/** Per-dashboard-socket viewer ref-count for live video, scoped by UI surface. */
...
function normalizeSurface(surface) {
    ...
    if (s === 'analytics-fr' || s === 'analytics_fr' || s === 'fr-live' || s === 'fr') return 'analytics-fr';
    if (s === 'analytics-anpr' || s === 'analytics_anpr' || s === 'anpr-live' || s === 'anpr') return 'analytics-anpr';
    if (s === 'analytics-weapon' || s === 'analytics_weapon' || s === 'weapon-live' || s === 'weapon') {
        return 'analytics-weapon';
    }
    if (s === 'matrix-popout' || s === 'matrix_popout') return 'matrix-popout';
    if (s === 'live-popout' || s === 'live_popout' || s === 'panel-popout') return 'live-popout';
    /* Tactical pin / select-zone — separate from Ops wall (do not steal ops viewer refs) */
    if (s === 'tactical' || s === 'tactical-pin' || s === 'tactical_pin') return 'tactical';
    return 'ops';
}
...
/** Per-socket surfaces holding a viewer ref for one cam. */
function socketSurfacesForCam(socketId, camId) {
    ...
    cams.forEach(function (count, key) {
        ...
        else if (parsed.surface === 'analytics-fr') out.analyticsFr = true;
        else if (parsed.surface === 'analytics-anpr') out.analyticsAnpr = true;
        else if (parsed.surface === 'analytics-weapon') out.analyticsWeapon = true;
        else if (parsed.surface === 'matrix-popout') out.matrixPopout = true;
        else if (parsed.surface === 'live-popout') out.livePopout = true;
        else if (parsed.surface === 'tactical') out.tactical = true;
        else out.ops = true;
    });
    return out;
}
...
function refBreakdownForCam(camId) {
    ...
    viewersBySocket.forEach(function (cams) {
        ...
        cams.forEach(function (count, key) {
            ...
            else if (parsed.surface === 'analytics-fr') analyticsFr += count;
            else if (parsed.surface === 'analytics-anpr') analyticsAnpr += count;
            else if (parsed.surface === 'analytics-weapon') analyticsWeapon += count;
            else if (parsed.surface === 'matrix-popout') matrixPopout += count;
            else if (parsed.surface === 'live-popout') livePopout += count;
            else if (parsed.surface === 'tactical') tactical += count;
            else ops += count;
        });
    ...
    return {
        ...
        analyticsFr: analyticsFr,
        analyticsAnpr: analyticsAnpr,
        analyticsWeapon: analyticsWeapon,
        matrixPopout: matrixPopout,
    ...
    camSet.forEach(function (camId) {
        ...
        if (sockets) {
            sockets.forEach(function (sock) {
                ...
                let hasAnalyticsFr = false;
                cams.forEach(function (count, key) {
                    if (count <= 0) return;
                    const parsed = parseViewKey(key);
                    if (parsed.camId !== camId) return;
                    if (parsed.surface === 'command-wall') hasCommandWall = true;
                    else if (parsed.surface === 'analytics-fr') hasAnalyticsFr = true;
                    else hasOps = true;
                });
                if (!hasOps && !hasCommandWall && !hasAnalyticsFr) return;
        ...
        cameras.push({
            ...
            analyticsFrRefs: refs.analyticsFr,
...
function listCamIdsForSurface(surface) {
    ...
    viewersBySocket.forEach(function (cams) {
        cams.forEach(function (count, key) {
            if (count <= 0) return;
            const parsed = parseViewKey(key);
            if (parsed.surface === want) ids.add(parsed.camId);
        });
    });
