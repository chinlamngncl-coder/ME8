...
function buildStreamReadyMsg(camId, surface, meta) {
    const msg = { camId: String(camId).trim(), surface: normalizeSurface(surface) };
    const extra = meta || {};
    if (extra.engine) msg.engine = extra.engine;
    if (extra.playUrl) msg.playUrl = extra.playUrl;
    return msg;
}

/** Emit video-stream-ready only to sockets that hold a viewer ref for this cam. */
function notifyStreamReady(io, camId, meta) {
    ...
    sockets.forEach((sock) => {
        ...
        surfaces.forEach((surface) => {
            if (sock.connected) {
                sock.emit('video-stream-ready', buildStreamReadyMsg(want, surface, meta));
                sent += 1;
            }
        });
    });
