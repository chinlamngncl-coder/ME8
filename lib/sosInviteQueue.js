/**
 * SOS live-video invite queue — max 8 concurrent streams; FIFO for the rest.
 */
const log = require('./fleetLog');

const MAX_SOS_LIVE = 8; /* SOS-LIVE-8-OVERRIDE-V1 */
const queue = [];
...
function slotsFree() {
    ...
    /* SOS-INVITE-GUARDS-V1 — a pending cam still waiting for contact has no pool session yet;
       count it, or a burst admits more than MAX_SOS_LIVE. */
    const live = new Set(pool.listCamIds());
    let reserved = 0;
    pending.forEach((id) => { if (!live.has(id)) reserved += 1; });
    return Math.max(0, MAX_SOS_LIVE - pool.countActive() - reserved);
}
