...
function createDashboardMediaWs(opts) {
    ...
    function bindUpgrade(server, mode) {
        ...
        /*
         * WSS-HTTPS-UPGRADE-LAN-V1 — Socket.IO/Engine.IO also listens for 'upgrade' and
         * abortUpgrade()s unknown paths (closes /ws/audio before open). Capture prior
         * listeners, remove them, and only forward non-media upgrades (e.g. /socket.io).
         * Re-run after io.attach so Engine.IO is inside `prior`, not a sibling destroyer.
         */
        const prior = server.listeners('upgrade')
            .filter((fn) => !fn || !fn.__me8MediaWsRouter)
            .slice();
        server.removeAllListeners('upgrade');

        function router(req, socket, head) {
            try {
                if (forceVideo) {
                ...
                for (let i = 0; i < prior.length; i++) {
                    try {
                        prior[i].call(server, req, socket, head);
                    } catch (_) { /* ignore */ }
                }
            } catch (err) {
        ...
        router.__me8MediaWsRouter = true;
        server.on('upgrade', router);
        server.__me8MediaWsUpgradeBound = true;
    }
