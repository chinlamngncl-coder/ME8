...
function resolveRedactionExe() {
    const name = process.platform === 'win32' ? 'redaction-engine.exe' : 'redaction-engine';
...
function runPython(pyBin, args, opts) {
    return new Promise((resolve) => {
        const exe = resolveRedactionExe();
        const cmd = exe || pyBin;
        const finalArgs = exe
            ? args
            : ((pyBin === 'py') ? ['-3', SCRIPT].concat(args) : [SCRIPT].concat(args));
        execFile(cmd, finalArgs, {
...
function engineArgs(engine) {
    ...
    if (!resolveRedactionExe() && fs.existsSync(YUNET_MODEL)) {
        args.push('--model', YUNET_MODEL);
    }
    return args;
}
...
async function selfCheck() {
    ...
    if (!exe && !fs.existsSync(SCRIPT)) {
        return { ok: false, error: 'sidecar script missing' };
    }
    const engine = redactFaceEngine();
    if (exe) {
        const res = await runPython(exe, ['selfcheck'].concat(engineArgs(engine)), { timeout: 20000 });
        const report = parseJsonLoose(res.stdout);
        if (report) {
            report.engine = report.engine || engine;
            return report;
        }
        return { ok: false, engine: engine, error: 'redaction engine failed' };
    }
    const tried = [];
...
async function detect(videoPath, opts) {
    ...
    const exe = resolveRedactionExe();
    if (exe) {
        const res = await runPython(exe, args, { timeout: opts.timeout || 300000 });
        const report = parseJsonLoose(res.stdout);
...
async function burnFaceFollow(videoPath, outPath, opts) {
    ...
    const args = [
        ...
        /* REDACT-HOLD-SMOOTH-V1 — hold 14→22 + coast in Python; pad stays 0.12 */
        ...
        '--hold-frames', String(opts.holdFrames != null ? opts.holdFrames : 22),
        '--iou', String(opts.iou != null ? opts.iou : 0.18),
    ]);
    if (opts.maxSeconds) args.push('--max-seconds', String(opts.maxSeconds));
    if (opts.timelineOut) args.push('--timeline-out', String(opts.timelineOut));
    if (opts.excludeJsonPath) args.push('--exclude-json', String(opts.excludeJsonPath));

    const exeBurn = resolveRedactionExe();
    if (exeBurn) {
        const res = await runPython(exeBurn, args, { timeout: opts.timeout || 900000 });
        const report = parseJsonLoose(res.stdout);
        if (report) return report;
        return { ok: false, engine: engine, error: 'face-follow burn failed' };
    }
