...
function envDefaults() {
    ...
    return {
        ...
        sip: {
            /* mob-fleet-sip-port-5062-v1 — Fleet YDT/telemetry SIP; WVP owns GB video on 5060 */
            sipPort: parseInt(process.env.FM_GB28181_SIP_PORT || '5062', 10),
...
function normalize(s) {
    ...
    return {
        ...
        sip: {
            /* Env wins over stale server-settings.json (dual-protocol port moves). */
            sipPort: (function () {
                const fromEnv = parseInt(process.env.FM_GB28181_SIP_PORT || '', 10);
                if (Number.isFinite(fromEnv) && fromEnv > 0 && fromEnv < 65536) return fromEnv;
                return parseInt(sipIn.sipPort != null ? sipIn.sipPort : merged.sipPort, 10) || 5062;
            })(),
        ...
        smtp: platformSmtp.normalizeSmtp(merged.smtp, platformSmtp.smtpDefaultsFromEnv()),
        evidence: {
            archivePrimary: storagePaths.normalizeArchivePrimary(
                merged.evidence && merged.evidence.archivePrimary
            ),
            /* VMS-LIVE-CAPTURE-SOS-DEFAULT-ON-V1 — product default ON forever; operator may turn OFF */
            liveCaptureDefaultOnV1: true,
            liveCaptureEnabled: (function () {
                const ev = merged.evidence || {};
                if (ev.liveCaptureDefaultOnV1 !== true) return true;
                if (ev.liveCaptureEnabled != null) return !!ev.liveCaptureEnabled;
                return true;
            })(),
            ...
            liveCaptureAutoOnSos: (function () {
                const ev = merged.evidence || {};
                if (ev.liveCaptureDefaultOnV1 !== true) return true;
                if (ev.liveCaptureAutoOnSos != null) return !!ev.liveCaptureAutoOnSos;
                return true;
            })(),
        ...
        frStorage: {
            rootPath: String((merged.frStorage && merged.frStorage.rootPath) || '').trim(),
            activeRootPath: String((merged.frStorage && merged.frStorage.activeRootPath) || '').trim(),
        },
        ...
        /* MAP-SOURCE-SETTINGS-TOGGLE-V1 — auto | online | local */
        mapSource: normalizeMapSource(merged.mapSource),
        /* DOCK-IDENTITY-FIRST-SETUP-HINT-V1 — Super admin once; no officer nag */
        dockIdentity: {
            setupAckAt: String((merged.dockIdentity && merged.dockIdentity.setupAckAt) || '').trim() || null,
            remindNextLogin: !!(merged.dockIdentity && merged.dockIdentity.remindNextLogin),
        },
        ptt: {
...
function normalizeMapSource(v) {
    const s = String(v || 'auto').toLowerCase().trim();
    return (s === 'online' || s === 'local') ? s : 'auto';
}
...
function load(storageDir) {
    ...
    /* One-shot: stamp default-ON policy so old sites (flags were false) record on SOS */
    const needsDefaultOnStamp = !(raw && raw.evidence && raw.evidence.liveCaptureDefaultOnV1 === true);
    if (needsDefaultOnStamp) {
        try {
            fs.mkdirSync(storageDir, { recursive: true });
            const secrets = serverSecrets.loadSecrets(storageDir);
            const stamped = serverSecrets.applySecrets(settings, secrets);
            fs.writeFileSync(
                filePath,
                JSON.stringify(serverSecrets.stripSecretsForDisk(stamped), null, 2),
                'utf8'
            );
            settings = stamped;
        } catch (_) { /* keep in-memory defaults */ }
    }
    const secrets = serverSecrets.loadSecrets(storageDir);
...
function publicView(settings) {
    ...
    const flags = serverSecrets.secretFlags({
        ...
        smtp: { password: s.smtp && s.smtp.password },
    });
    if (s.sip) {
    ...
    if (s.smtp) {
        s.smtp = platformSmtp.smtpPublicView(settings);
    }
    return s;
}
    ...
    const rows = [
        ...
        },
        {
            ...
            note: transport === 'udp'
                ...
                : 'TCP 40131 when media transport is set to TCP in Server Config',
            required: true,
        },
        {
            service: 'SOS group Call RTP',
            port: `${r.sosGroupCallRtpBase || 40400}–${(r.sosGroupCallRtpBase || 40400) + 7}`,
            protocol: 'udp',
            direction: 'inbound',
            audience: 'BWCs',
            note: 'Eight dedicated PCMA ports for simultaneous SOS group Call members',
            required: false,
        },
    ];
    if (r.pttEnabled) {
        rows.push({
            ...
            direction: 'inbound',
            audience: 'BWCs',
            note: 'Push-to-talk cluster (when PTT is enabled on this server)',
            required: false,
        });
    }
    if (r.ftpEnabled) {
