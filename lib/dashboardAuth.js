...
const DEFAULT_PERMISSIONS = Object.freeze({
    ...
    clearMapPins: false,
    evidenceView: false,
    evidenceTriageAccess: false,
    evidenceDownload: false,
    evidenceExport: false,
    evidenceEdit: false,
    evidenceLifecycle: false,
    dockAdmin: false,
...
/** Same floor for operator and super admin (MOB: unify-12-both-roles). */
const MIN_PASSWORD_LEN_SUPER_ADMIN = MIN_PASSWORD_LEN;
...
const PASSWORD_CHANGE_ALLOWED_PATHS = new Set([
    ...
    '/recovery-email.html',
    '/js/recovery-email-page.js',
    '/js/operator-error-voice.js',
    '/js/operator-ui.js',
    '/js/auth-form-busy.js',
    '/api/auth/recovery-email/status',
    '/api/auth/recovery-email/request',
    '/api/auth/recovery-email/resend',
]);
...
function getPasswordPolicyPublic(role) {
    ...
    return {
        minLength,
        minLengthSuperAdmin: MIN_PASSWORD_LEN,
...
function applyUserPasswordChange(user, newPassword, context) {
    ...
    if (!hashed || typeof hashed.hash !== 'string' || typeof hashed.hash.then === 'function') {
        throw new Error('Password hash failed');
    }
    user.salt = hashed.salt;
    user.passwordHash = hashed.hash;
    user.mustChangePassword = false;
    console.log('[auth] applyUserPasswordChange', { hashLen: user.passwordHash.length });
    return user;
}
...
function hashPassword(password, salt) {
    ...
    const digest = crypto.scryptSync(String(password), String(s), 64);
    const hash = Buffer.isBuffer(digest) ? digest.toString('hex') : String(digest || '');
    if (typeof hash !== 'string' || hash.length !== 128) {
        throw new Error('Password hash failed');
    }
    return { salt: String(s), hash: hash };
}

function storedPasswordHash(user) {
    const h = user && user.passwordHash;
    if (!h) return '';
    if (typeof h.then === 'function') return '';
    if (typeof h === 'string') return h;
    if (typeof h === 'object' && typeof h.hash === 'string') return h.hash;
    return '';
}

function storedPasswordSalt(user) {
    const s = user && user.salt;
    if (!s) return '';
    if (typeof s === 'string') return s;
    if (typeof s === 'object' && typeof s.salt === 'string') return s.salt;
    return '';
}
...
function readUsersFile() {
    try {
        ...
        console.log('[auth] readUsersFile failed', { hashLen: 0, reason: 'parse' });
        const wrap = new Error('USER_STORE_UNREADABLE');
        wrap.cause = err;
        throw wrap;
    }
    return null;
...
function writeUsersFile(users) {
    if (!usersPath) {
        console.log('[auth] writeUsersFile skipped: store not initialized');
        throw new Error('User store is not initialized');
    }
    const list = Array.isArray(users) ? users : [];
    list.forEach((u) => {
        if (!u) return;
        const h = storedPasswordHash(u);
        if (u.passwordHash && typeof u.passwordHash !== 'string') {
            if (!h) throw new Error('Refusing to save invalid password hash');
            u.passwordHash = h;
        }
        const s = storedPasswordSalt(u);
        if (u.salt && typeof u.salt !== 'string') {
            if (!s) throw new Error('Refusing to save invalid password salt');
            u.salt = s;
        }
        console.log('[auth] persist user hash', {
            hashLen: storedPasswordHash(u).length,
            saltLen: storedPasswordSalt(u).length,
        });
    });
    ...
    try {
        ...
        fs.copyFileSync(tmp, usersPath);
        try { fs.unlinkSync(tmp); } catch (e) { /* ignore */ }
    }
    console.log('[auth] wrote dashboard-users.json', { count: list.length });
}
...
function ensureUsers() {
    ...
    try {
        ...
        if (err && err.message === 'USER_STORE_UNREADABLE') {
            console.log('[auth] users file unreadable; not resetting factory account');
            throw new Error('User store could not be read');
        }
        throw err;
    }
...
function normalizePermissions(raw, role) {
    ...
    return {
        ...
        tacticalView: !!p.tacticalView,
        aiAlerts: !!p.aiAlerts,
        blueprintManage: !!p.blueprintManage,
...
function canEvidenceTriageAccess(session) {
    if (!session || !session.userId) return false;
    const user = findUserById(session.userId);
    if (normalizeRole(session.role) === 'super_admin') return true;
    if (normalizeRole(user && user.role) === 'super_admin') return true;
    return !!permissionsForUser(user).evidenceTriageAccess;
}

function requireEvidenceTriageAccess(req, res, next) {
    const session = req.dashboardUser || sessionFromRequest(req);
    if (!session) return res.status(401).json({ ok: false, error: 'Unauthorized' });
    if (!canEvidenceTriageAccess(session)) {
        return res.status(403).json({ ok: false, error: 'Evidence triage access required' });
    }
    return next();
}
...
function canEvidenceLifecycle(session) {
    if (!session || !session.userId) return false;
    const user = findUserById(session.userId);
    if (normalizeRole(user && user.role) === 'super_admin') return true;
    return !!permissionsForUser(user).evidenceLifecycle;
}

function requireEvidenceLifecycle(req, res, next) {
    const session = req.dashboardUser || sessionFromRequest(req);
    if (!session || !canEvidenceLifecycle(session)) {
        return res.status(403).json({ ok: false, error: 'Evidence Lifecycle permission required' });
    }
...
function canTacticalView(session) {
    if (!session || !session.userId) return false;
    const user = findUserById(session.userId);
    if (normalizeRole(user && user.role) === 'super_admin') return true;
    return !!permissionsForUser(user).tacticalView;
}

function canAiAlerts(session) {
    if (!session || !session.userId) return false;
    const user = findUserById(session.userId);
    if (normalizeRole(user && user.role) === 'super_admin') return true;
    return !!permissionsForUser(user).aiAlerts;
}

function canBlueprintManage(session) {
    if (!session || !session.userId) return false;
    const user = findUserById(session.userId);
    if (normalizeRole(user && user.role) === 'super_admin') return true;
    return !!permissionsForUser(user).blueprintManage;
}
...
function provisionOidcUser(username, role) {
    ...
    platformLimits.assertOperatorCount(countOperators(users) + 1);
...
function verifyPassword(user, password) {
    if (!user) return false;
    const stored = storedPasswordHash(user);
    const salt = storedPasswordSalt(user);
    if (!stored || !salt) {
        console.log('[auth] compare skipped', { ok: false, hashLen: stored.length, hasSalt: !!salt });
        return false;
    }
    ...
    console.log('[auth] compare', { ok: ok, hashLen: stored.length, computedLen: hash.length });
    return ok;
}
...
function countOperators(users) {
    return users.filter((u) => u.active !== false && normalizeRole(u.role) === 'operator').length;
}

function createUser({ username, password, role, assignedGroupIds, displayName, contactNote, permissions }) {
    const name = String(username || '').trim();
    const pass = String(password || '');
    const r = normalizeRole(role);
    if (r === 'super_admin') {
    ...
    if (r === 'operator' && permissions && typeof permissions === 'object') {
        user.permissions = normalizePermissions(permissions, 'operator');
    }
    ...
    users.push(user);
    saveUsers(users);
    const fresh = findUserByUsername(name);
    const persistOk = verifyPassword(fresh, pass);
    console.log('[auth] createUser persist-check', {
        ok: persistOk,
        hashLen: storedPasswordHash(fresh).length,
    });
    if (!persistOk) {
        throw new Error('Password was saved but could not be verified');
    }
    return {
        id: user.id,
        username: user.username,
...
function updateUser(userId, patch) {
    ...
    const wasOperator = normalizeRole(user.role) === 'operator';
    const willOperator = nextRole === 'operator';
    if (!wasOperator && willOperator) {
        const platformLimits = require('./platformLimits');
        platformLimits.assertOperatorCount(countOperators(users) + 1);
    }
    ...
    if (patch.permissions != null && typeof patch.permissions === 'object') {
        if (normalizeRole(user.role) === 'super_admin') {
            ...
            user.permissions = {
                ...
                evidenceTriageAccess: !!patch.permissions.evidenceTriageAccess,
                ...
                evidenceLifecycle: !!patch.permissions.evidenceLifecycle,
                ...
                tacticalView: !!patch.permissions.tacticalView,
                aiAlerts: !!patch.permissions.aiAlerts,
                blueprintManage: !!patch.permissions.blueprintManage,
...
function changeOwnPassword(username, currentPassword, newPassword) {
    ...
    saveUsers(users);
    const fresh = findUserById(users[idx].id);
    const persistOk = verifyPassword(fresh, newPassword);
    console.log('[auth] changeOwnPassword persist-check', {
        ok: persistOk,
        hashLen: storedPasswordHash(fresh).length,
    });
    if (!persistOk) {
...
const PUBLIC_PATHS = new Set([
    ...
    '/api/health',
    '/verify-recovery-email.html',
    '/js/verify-recovery-email.js',
    '/api/auth/recovery-email/confirm',
]);

/**
 * Public login-page hints. Factory password text must NEVER show for returning sites.
 * True only while the default install account still requires first password change.
 */
function getLoginUiPublicHints() {
    const user = findUserByUsername(DEFAULT_USERNAME);
    const showFactoryPasswordHint = !!(
        user &&
        user.active !== false &&
        user.mustChangePassword === true
    );
    return { showFactoryPasswordHint: showFactoryPasswordHint };
}
...
function isPublicPath(reqPath) {
    ...
    if (process.env.FM_LAB_ZLM === '1') {
        if (reqPath.startsWith('/api/lab/zlm/flv/')) return true;
        if (reqPath === '/api/lab/zlm/b1/flv-url') return true;
    }
    /* mob-track-b1-flv-auth-allow — mpegts FLV fetch has no cookie; token is the gate */
    if (String(process.env.FM_LAB_WVP || '').trim() === '1'
        || String(process.env.FM_WVP_VIDEO_HANDOFF || '').trim() === '1') {
        if (reqPath === '/api/lab/wvp/flv' || reqPath.startsWith('/api/lab/wvp/flv?')) return true;
        if (reqPath === '/api/lab/wvp/flv-stream') return true;
    }
    return false;
}
...
function sessionMustVerifyRecoveryEmail(session) {
    if (!session || !session.userId) return false;
    return authRecoveryEmail.mustVerifyRecoveryEmail(findUserById(session.userId));
}

function recoveryEmailSessionPayload(user) {
    const status = authRecoveryEmail.recoveryStatus(user || {});
    return {
        recoveryEmail: status.email || null,
        recoveryEmailVerified: status.verified,
        recoveryEmailVerifiedAt: status.verifiedAt,
        recoveryEmailPending: status.pending,
        mustVerifyRecoveryEmail: authRecoveryEmail.mustVerifyRecoveryEmail(user),
    };
}

function beginRecoveryEmailVerify(userId, email) {
    const users = ensureUsers();
    const user = users.find(function (u) { return u && u.id === userId; });
    if (!user) return null;
    const token = authRecoveryEmail.prepareUserPendingVerify(user, email);
    saveUsers(users);
    return { user: user, token: token };
}

function completeRecoveryEmailVerify(token) {
    const users = ensureUsers();
    const user = authRecoveryEmail.findUserByVerifyToken(users, token);
    if (!user) return null;
    authRecoveryEmail.confirmUserVerify(user);
    saveUsers(users);
    return user;
}
...
function requireDashboardAuth(req, res, next) {
    ...
    if (session) {
        ...
        } else if (sessionMustVerifyRecoveryEmail(session)) {
            if (!isPasswordChangeBypassPath(req.path)) {
                if (req.path.startsWith('/api/')) {
                    return res.status(403).json({
                        ok: false,
                        error: 'Recovery email verification required before using the dashboard',
                        errorKey: 'errors.recoveryEmailRequired',
                        mustVerifyRecoveryEmail: true,
                    });
                }
                if (req.path === '/' || req.path === '/index.html' || req.path.endsWith('.html')) {
                    return res.redirect('/recovery-email.html');
                }
                return res.status(403).send('Recovery email verification required');
            }
        }
        return next();
    }
    ...
    if (req.path === '/' || req.path === '/index.html' || req.path.endsWith('.html')) {
        const ret = encodeURIComponent(req.originalUrl || req.url || '/');
        return res.redirect('/login.html?return=' + ret);
    }
...
module.exports = {
    ...
    canEvidenceDownload,
    canEvidenceView,
    canEvidenceTriageAccess,
    requireEvidenceTriageAccess,
    canEvidenceExport,
    canEvidenceEdit,
    canEvidenceLifecycle,
    requireEvidenceLifecycle,
    canDockAdmin,
    canConferenceView,
    canConferenceJoin,
    canConferenceHost,
    ...
    sessionMustVerifyRecoveryEmail,
    recoveryEmailSessionPayload,
    beginRecoveryEmailVerify,
    completeRecoveryEmailVerify,
    userMustChangePassword,
    assertPasswordAllowed,
    getLoginUiPublicHints,
    DEFAULT_PASSWORD,
};
