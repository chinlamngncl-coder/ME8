...
function buildSummary(deps) {
    ...
    const auditRaw = deps.getAuditEntries ? deps.getAuditEntries(40) : [];
    ...
    const deviceCapacity = Math.max(1, parseInt(deps.deviceCapacity, 10) || 1);
    const volume = deps.volume && typeof deps.volume === 'object' ? deps.volume : {};
    const volTotal = Number(volume.totalBytes) || 0;
    const volUsed = Number(volume.usedBytes) || 0;
    const volFree = Number(volume.freeBytes) || 0;
    const storageUsedPct = volTotal > 0
        ? ceilPct(volUsed, volTotal)
        : 0;
    const registered = fleet.length || (online.length + offline.length);
    const onlinePct = registered > 0 ? ceilPct(online.length, registered) : 0;
    const aiHits = deps.aiHits && typeof deps.aiHits === 'object' ? deps.aiHits : {};
    const docks = Array.isArray(deps.docks) ? deps.docks : [];
    ...
    return {
        ...
        storage: {
            totalBytes: volUsed || totalStorageBytes,
            totalLabel: volTotal
                ? (formatBytes(volUsed) + ' / ' + formatBytes(volTotal))
                : formatBytes(totalStorageBytes),
            freeLabel: volTotal ? formatBytes(volFree) : '',
        ...
        aiHits: {
            frToday: Number(aiHits.frToday) || 0,
            anprToday: Number(aiHits.anprToday) || 0,
            frTotal: Number(aiHits.frTotal) || 0,
            anprTotal: Number(aiHits.anprTotal) || 0,
        },
        docks: docks,
        services: deps.serviceStatus || {},
        recentActivity: audit.slice(0, 10).map((e) => ({
            at: e.ts || e.at,
            action: e.action,
