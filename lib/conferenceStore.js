...
/* JSON-STORE-SINGLE-WRITER-V1 — cached + atomic temp/rename write (shared helper). */
const confStore = require('./jsonStoreWriter').createStore({
    path: () => storePath,
    parse: (data) => (data && data.rooms ? data : null),
    fallback: defaultStore,
});

function readStore() {
    if (!storePath) return defaultStore();
    return confStore.read();
}

function writeStore(data) {
    if (!storePath) return;
    confStore.write(data);
}
