...
function installFatalProcessPolicy(options = {}) {
    ...
    function terminate(kind, value) {
        ...
        // Advanced Telemetry Engine: Heuristic Trace Analysis → system_anomalies (blocking)
        try {
            const telemetryRecorder = require('../utils/telemetryRecorder');
            telemetryRecorder.persistFatalBlocking(kind, value);
        } catch (_) {
            // Never block termination on recorder failure.
        }
