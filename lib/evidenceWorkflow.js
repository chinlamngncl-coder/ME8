...
const OFFICER_TRIAGE_TAG = 'ax-officer-triage';
...
async function getDetail(fileId, sosLookup) {
    const file = await evidenceRegistry.getFile(fileId);
    if (!file) return null;
    const meta = await siteDb.getEvidenceMeta(fileId) || {
    ...
    const attachments = await siteDb.listEvidenceAttachments(fileId);
    const exports = await siteDb.listEvidenceExports(fileId, 20);
    let sos = null;
    ...
    return {
        file: Object.assign({}, file, {
            ...
            queuedDelete: !!file.queuedDelete,
        }),
        meta: meta,
        cryptoStatus: resolveCryptoStatus(file),
        archived: evidenceRegistry.isArchivedFile(file),
        queuedDelete: !!file.queuedDelete,
        deleteQueue: file.deleteQueue || null,
...
async function updateMeta(fileId, patch, actor) {
    const file = await evidenceRegistry.getFile(fileId);
    ...
    if (patch.tags !== undefined) {
        ...
        if ((prev.tags || []).indexOf(OFFICER_TRIAGE_TAG) >= 0 && tags.indexOf(OFFICER_TRIAGE_TAG) < 0
            && patch.removeFromTriage !== true) {
            tags.push(OFFICER_TRIAGE_TAG);
        }
    }
    const next = {
        evidenceFileId: fileId,
    ...
    await siteDb.upsertEvidenceMeta(next);
    return siteDb.getEvidenceMeta(fileId);
}

async function setOfficerTriage(fileIds, actor, on) {
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        const prev = await siteDb.getEvidenceMeta(fileId) || { evidenceFileId: fileId, tags: [] };
        let tags = Array.isArray(prev.tags) ? prev.tags.slice() : [];
        tags = tags.filter((t) => String(t) !== OFFICER_TRIAGE_TAG);
        if (on) tags.push(OFFICER_TRIAGE_TAG);
        await siteDb.upsertEvidenceMeta({
            evidenceFileId: fileId,
            notes: prev.notes || '',
            sosIncidentId: prev.sosIncidentId || null,
            trimStartSec: prev.trimStartSec,
            trimEndSec: prev.trimEndSec,
            dockId: prev.dockId || null,
            dockBay: prev.dockBay,
            tags: tags,
            updatedBy: actor && (actor.username || actor.displayName) ? (actor.username || actor.displayName) : null,
        });
        out.push(fileId);
    }
    return { ok: true, ids: out, count: out.length };
}

async function saveAttachment(fileId, originalName, buffer, actor) {
    const file = await evidenceRegistry.getFile(fileId);
    if (!file) throw new Error('Evidence file not found');
    ...
    await siteDb.insertEvidenceAttachment({
        id: id,
        evidenceFileId: fileId,
        fileName: safeName,
        relativePath: rel,
...
async function resolveAttachmentStream(attachmentId) {
    const att = await siteDb.getEvidenceAttachment(attachmentId);
    if (!att) return null;
...
async function runTrimExport(fileId, opts, actor) {
    ...
    await siteDb.insertEvidenceExport({
        exportId: exportId,
        evidenceFileId: fileId,
        exportType: 'trim',
        relativePath: rel,
    ...
    const priorMeta = await siteDb.getEvidenceMeta(fileId) || {};
    await siteDb.upsertEvidenceMeta({
        evidenceFileId: fileId,
        trimStartSec: startSec,
        trimEndSec: endSec,
        notes: opts.notes != null ? String(opts.notes) : priorMeta.notes,
        sosIncidentId: opts.sosIncidentId !== undefined ? opts.sosIncidentId : priorMeta.sosIncidentId,
        updatedBy: actor && actor.username ? actor.username : null,
    });
...
/**
 * Lossless clip into a NEW catalog file. Never overwrites or deletes the master.
 */
async function runTrimClip(fileId, opts, actor) {
    const file = await evidenceRegistry.getFile(fileId);
    if (!file) throw new Error('Evidence file not found');
    const inputPath = evidenceRegistry.resolveFilePath(file);
    if (!inputPath) throw new Error('Source file missing on storage');

    let startSec = opts.startSec != null ? Number(opts.startSec) : (opts.trimStartSec != null ? Number(opts.trimStartSec) : 0);
    let endSec = opts.endSec != null ? Number(opts.endSec) : (opts.trimEndSec != null ? Number(opts.trimEndSec) : null);
    if (Number.isNaN(startSec) || startSec < 0) startSec = 0;
    ...
    if (duration != null) {
        ...
        throw new Error('Set a valid trim end time');
    }
    if (endSec <= startSec) throw new Error('Trim end must be after start');
    if ((endSec - startSec) < MIN_TRIM_SECONDS) {
        throw new Error('Trimmed clip must be at least ' + MIN_TRIM_SECONDS + ' seconds long');
    ...
    if (!archiveRoot) throw new Error('Evidence archive is not ready');
    const destDir = path.join(archiveRoot, 'clips');
    fs.mkdirSync(destDir, { recursive: true });
    const stamp = Date.now().toString(36);
    const outName = path.basename(file.fileName, path.extname(file.fileName))
        + '_clip_' + stamp + '.mp4';
    const outPath = path.join(destDir, outName);
    if (path.resolve(outPath) === path.resolve(inputPath)) {
        throw new Error('Clip path must not replace the master file');
    }

    const ff = resolveFfmpeg.resolveFfmpegPath();
    const muteAudio = !!(opts.muteAudio || opts.mute);
    const args = muteAudio
        ? [
            '-hide_banner',
            '-y',
            '-i', inputPath,
    ...
    try {
        ...
        try { fs.unlinkSync(outPath); } catch (_) { /* new file only */ }
        throw new Error(ffmpegTrimUserMessage(err));
    }
    if (!fs.existsSync(outPath)) throw new Error('Clip was not written');
    if (path.resolve(outPath) === path.resolve(inputPath)) {
        throw new Error('Master file path collision');
    }
    let byteSize = 0;
    try { byteSize = fs.statSync(outPath).size; } catch (_) { byteSize = 0; }
    if (byteSize < 1024) {
        try { fs.unlinkSync(outPath); } catch (_) { /* new file only */ }
        throw new Error('Trimmed clip came out empty — try a longer range');
    }

    const sha256 = await hashFileSha256(outPath);
    ...
    if (!clipId) {
        try { fs.unlinkSync(outPath); } catch (_) { /* new file only */ }
        throw new Error('Clip could not enter the evidence catalog');
    }
...
/** REDACT-PREVIEW-CONTROLS-BURN-V1 — deleted Auto preview boxes (empty OK). */
function validateExcludeRegions(regions) {
...
async function applyRedaction(fileId, regions, actor, opts) {
    opts = opts || {};
    const src = await resolveRedactInputPath(fileId, opts);
    const file = src.file;
    ...
    const metaJson = JSON.stringify({
        ...
        mode: src.secondPass ? 'second-pass' : 'static-regions',
...
async function applyFaceFollowRedaction(fileId, actor, opts) {
    ...
    const excludeRegions = validateExcludeRegions(opts.excludeRegions);
    ...
    const excludeJsonPath = path.join(tmpDir, 'exclude.json');

    try {
        if (excludeRegions.length) {
            fs.writeFileSync(excludeJsonPath, JSON.stringify(excludeRegions), 'utf8');
        }
        const burn = await faceTrackSidecar.burnFaceFollow(inputPath, burnedPath, {
            score: opts.score != null ? opts.score : 0.72,
            /* pad stays tight — operator: bigger box covers body */
            pad: opts.pad != null ? opts.pad : 0.12,
            sigma: opts.sigma != null ? opts.sigma : 18,
            detectEvery: opts.detectEvery != null ? opts.detectEvery : 1,
            holdFrames: opts.holdFrames != null ? opts.holdFrames : 22,
            iou: opts.iou != null ? opts.iou : 0.18,
            maxSeconds: opts.maxSeconds || 0,
            timeout: opts.timeout || 900000,
            excludeJsonPath: excludeRegions.length ? excludeJsonPath : null,
        });
        if (!burn || !burn.ok) {
        ...
        const metaJson = JSON.stringify({
            sourceFileId: fileId,
            sourceFileName: file.fileName,
            mode: src.secondPass ? 'second-pass-face-follow' : 'face-follow',
            secondPass: !!src.secondPass,
            parentExportId: src.parentExportId || null,
            faceFollow: {
                ...
                excludeCount: excludeRegions.length,
                excludedHits: burn.excludedHits || 0,
            },
...
async function getRedactExport(exportId) {
    const row = await siteDb.getEvidenceExport(exportId);
    if (!row || row.exportType !== 'redact') return null;
    return row;
}

async function saveRedactNote(exportId, patch, actor) {
    const row = await getRedactExport(exportId);
    ...
    await siteDb.updateEvidenceExportMeta(exportId, next);
    return { exportId: exportId, meta: next };
}

async function finalizeRedactExport(exportId, actor) {
    const row = await getRedactExport(exportId);
    if (!row) throw new Error('Redacted export not found');
    const meta = parseExportMeta(row);
...
async function resolveExportStream(exportId) {
    const row = await siteDb.getEvidenceExport(exportId);
    if (!row) return null;
...
/**
 * REDACT-SECOND-PASS-ON-EXPORT-V1 — input is parent redacted export MP4, not original.
 * Still tied to the same evidenceFileId for catalog / Prior exports.
 */
async function resolveRedactInputPath(fileId, opts) {
    opts = opts || {};
    ...
    const file = await evidenceRegistry.getFile(fileId);
    if (!file) throw new Error('Evidence file not found');
    const resolved = await resolveExportStream(parentExportId);
    if (!resolved) throw new Error('Parent redacted export not found');
    if (String(resolved.row.evidenceFileId) !== String(fileId)) {
        throw new Error('Export does not belong to this evidence file');
    }
    if (resolved.row.exportType !== 'redact') {
        throw new Error('Only a redacted export can be used for second pass');
    }
    return {
        file: file,
        inputPath: resolved.fullPath,
...
/**
 * REDACT-PRIOR-EXPORTS-CLEANUP-V1
 * Remove one non-finalized redacted export (catalog + disk). Original evidence unchanged.
 */
async function removeRedactExportDraft(exportId) {
    const row = await siteDb.getEvidenceExport(exportId);
    if (!row) throw new Error('Export not found');
    if (row.exportType !== 'redact') throw new Error('Only redacted draft exports can be removed');
    const status = (row.meta && row.meta.status) || '';
    if (status === 'finalized') throw new Error('Finalized exports cannot be removed');

    const abs = exportAbs(row.relativePath);
    if (abs && fs.existsSync(abs)) {
        try { fs.unlinkSync(abs); } catch (_) { /* DB row still removed */ }
    }
    const ok = await siteDb.deleteEvidenceExport(exportId);
    if (!ok) throw new Error('Export not found');
    return {
...
/**
 * Remove all non-finalized redacted exports for one evidence file.
 */
async function removeRedactExportDraftsForFile(fileId) {
    const id = String(fileId || '').trim();
    if (!id) throw new Error('Evidence file required');
    const rows = await siteDb.listNonFinalizedRedactExports(id);
    const removed = [];
    for (let i = 0; i < rows.length; i++) {
        const out = await removeRedactExportDraft(rows[i].exportId);
        removed.push(out);
    }
    return {
        evidenceFileId: id,
...
/**
 * REDACT-PRIOR-EXPORTS-CLEAR-FINALIZED-V1
 * Remove one finalized redacted export (catalog + disk). Original evidence unchanged.
 */
async function removeRedactExportFinalized(exportId) {
    const row = await siteDb.getEvidenceExport(exportId);
    if (!row) throw new Error('Export not found');
    if (row.exportType !== 'redact') throw new Error('Only redacted exports can be removed');
    const status = (row.meta && row.meta.status) || '';
    if (status !== 'finalized') throw new Error('Export is not finalized');
...
/**
 * Remove all finalized redacted exports for one evidence file.
 */
async function removeRedactExportFinalizedForFile(fileId) {
    const id = String(fileId || '').trim();
    if (!id) throw new Error('Evidence file required');
    const rows = await siteDb.listFinalizedRedactExports(id);
    const removed = [];
    for (let i = 0; i < rows.length; i++) {
        const out = await removeRedactExportFinalized(rows[i].exportId);
        removed.push(out);
    }
    return {
        evidenceFileId: id,
...
/**
 * PRIOR-TRIM-LABEL-AND-REMOVE-V1 — remove one trim export (catalog + disk). Original unchanged.
 */
async function removeTrimExport(exportId) {
    const row = await siteDb.getEvidenceExport(exportId);
    if (!row) throw new Error('Export not found');
    if (row.exportType !== 'trim') throw new Error('Only trim exports can be removed');
...
/**
 * Remove all trim exports for one evidence file.
 */
async function removeTrimExportsForFile(fileId) {
    const id = String(fileId || '').trim();
    if (!id) throw new Error('Evidence file required');
    const rows = await siteDb.listTrimExports(id);
    const removed = [];
    for (let i = 0; i < rows.length; i++) {
        const out = await removeTrimExport(rows[i].exportId);
        removed.push(out);
    }
    return {
        evidenceFileId: id,
...
async function resolvePreviewPath(fileId) {
    const file = await evidenceRegistry.getFile(fileId);
    if (!file) return null;
...
module.exports = {
    ...
    resolveRedactInputPath,
    saveRedactNote,
    finalizeRedactExport,
    getRedactExport,
    removeRedactExportDraft,
    removeRedactExportDraftsForFile,
    removeRedactExportFinalized,
    removeRedactExportFinalizedForFile,
    removeTrimExport,
    removeTrimExportsForFile,
    resolveExportStream,
    listRedactedExports,
    resolvePreviewPath,
    probeDuration,
