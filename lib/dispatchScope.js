...
/** Implicit zoned access — siteId matches assigned group id/name; unzoned (`default`) stays visible. */
function filterBlueprintsForUser(blueprints, user, groups) {
    const scope = scopeForUser(user, groups);
    const list = Array.isArray(blueprints) ? blueprints : [];
    if (scope.seeAll) return list;
    const idSet = new Set(scope.groupIds || []);
    const nameSet = new Set();
    (groups || []).forEach(function (g) {
        if (g && idSet.has(g.id)) nameSet.add(String(g.name || '').trim().toLowerCase());
    });
    return list.filter(function (bp) {
        const site = String((bp && bp.siteId) || '').trim();
        if (!site || site.toLowerCase() === 'default') return true;
        if (idSet.has(site)) return true;
        if (nameSet.has(site.toLowerCase())) return true;
        return false;
    });
}
