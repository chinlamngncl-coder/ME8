...
/* MAP-OPS-LOCAL-ZOOM-AND-FILL-V1 — pack ceiling is the highest tiles/{z} folder. Leaflet must
   not request z19 when the pack stops at ~14 (zoom-in went blank). Number only — never a path. */
function detectRasterMaxZoom() {
    const tilesRoot = safeJoin(GIS_OFFLINE_ROOT, 'tiles');
    if (!tilesRoot || !fs.existsSync(tilesRoot)) return null;
    let max = 0;
    try {
        fs.readdirSync(tilesRoot, { withFileTypes: true }).forEach((ent) => {
            if (!ent.isDirectory()) return;
            const z = parseInt(ent.name, 10);
            if (!Number.isFinite(z) || z < 0 || z > 22) return;
            if (String(z) !== ent.name) return;
            if (z > max) max = z;
        });
    } catch (_) {
        return null;
...
let getMapSourceFn = null;

function resolveMapSource() {
    try {
        if (typeof getMapSourceFn === 'function') {
            const v = String(getMapSourceFn() || 'auto').toLowerCase().trim();
            if (v === 'online' || v === 'local') return v;
        }
    } catch (_) { /* ignore */ }
    return 'auto';
}

function registerGisOfflineRoutes(app, opts) {
    if (opts && typeof opts.getMapSource === 'function') getMapSourceFn = opts.getMapSource;
    ...
    app.get('/api/gis/offline/config', (req, res) => {
        ...
        res.json({
            ...
            maxNativeZoom: detectRasterMaxZoom(),
            mapSource: resolveMapSource(),
        ...
        const style = {
            ...
            sources: {
                ...
                'fm-offline-raster': {
                    ...
                    maxzoom: detectRasterMaxZoom() || 14,
