...
function startWvpFlvToRtmp(session, sourceUrl, rtmpUrl) {
    stopFfmpegOnly(session);
    const gen = session.ffmpegGen;
    if (!ffmpegPath) ffmpegPath = resolveFfmpegPath(deps && deps.baseDir);
    const args = [
        '-hide_banner', '-loglevel', 'warning',
        // VC-BWC-INGRESS-JERK-SMOOTH-V1 — WVP→RTMP only; no +nobuffer (drops → micro-jerks).
    ...
    session.ffmpegAttached = true;
    session.ffmpegProcess.on('error', function (err) {
        if (session.ffmpegGen !== gen || !sessions.has(session.camId)) return;
        log.media.warn('conference WVP ffmpeg spawn failed', {
            camId: session.camId,
    ...
    session.ffmpegProcess.stderr.on('data', function (chunk) {
        const line = String(chunk).trim();
        markPipeDecoded(session, 'flv', line);
        if (line && /error/i.test(line)) {
            log.media.warn('conference WVP ffmpeg', {
                camId: session.camId,
                msg: line.slice(0, 200),
            });
        }
    });
    session.ffmpegProcess.stdout.on('data', function (chunk) {
        markPipeDecoded(session, 'flv', String(chunk));
    });
    session.ffmpegProcess.on('exit', function (code) {
        if (session.ffmpegGen !== gen || !sessions.has(session.camId)) return;
        log.media.warn('conference WVP ffmpeg exited — stopping session', {
            camId: session.camId,
            code,
        });
        stopSession(session.camId, { remote: false });
        if (onRemoteStop) {
            try { onRemoteStop(session.camId, { reason: 'ffmpeg_exit', code }); } catch (_) { /* ignore */ }
        }
    });
}

function startRtspToRtmp(session, sourceUrl, rtmpUrl, options) {
    stopFfmpegOnly(session);
    const gen = session.ffmpegGen;
    const transport = String(options && options.rtspTransport || 'tcp').toLowerCase() === 'udp'
        ? 'udp'
        : 'tcp';
    if (!ffmpegPath) ffmpegPath = resolveFfmpegPath(deps && deps.baseDir);
    const args = [
    ...
    session.ffmpegProcess.on('error', function (err) {
        if (session.ffmpegGen !== gen || !sessions.has(session.camId)) return;
        log.media.warn('conference fixed camera ffmpeg spawn failed', {
            cameraId: session.fixedCamId,
            message: err && err.message ? err.message : String(err),
        });
    });
    session.ffmpegProcess.stderr.on('data', function () {
        /* Credential-bearing RTSP URLs must never enter logs. */
    });
    session.ffmpegProcess.stdout.on('data', function (chunk) {
        markPipeDecoded(session, 'rtsp', String(chunk));
    });
    session.ffmpegProcess.on('exit', function (code) {
        if (session.ffmpegGen !== gen || !sessions.has(session.camId)) return;
        log.media.warn('conference fixed camera ffmpeg exited — stopping session', {
            cameraId: session.fixedCamId,
            code,
        });
        stopSession(session.camId, { remote: false });
...
function stopSession(camId, opts) {
    ...
    if (session.sourceWatchdog) {
        clearTimeout(session.sourceWatchdog);
        session.sourceWatchdog = null;
    }
    stopFfmpegOnly(session);
...
async function startFromWvpFlv(camId, rtmpUrl, sourceUrl) {
    if (!camId || !rtmpUrl || !sourceUrl) {
        throw new Error('camId, rtmpUrl and WVP FLV source are required');
    }
    if (sessions.has(camId)) stopSession(camId);
    const session = {
        camId,
    ...
    sessions.set(camId, session);
    startWvpFlvToRtmp(session, sourceUrl, rtmpUrl);
    session.sourceWatchdog = setTimeout(function () {
        if (!sessions.has(camId) || session.pipeDecoded) return;
        log.media.warn('conference WVP FLV: no decoded video, stopping session', { camId });
        stopSession(camId, { remote: false });
        if (onRemoteStop) {
            try { onRemoteStop(camId, { reason: 'wvp_flv_timeout' }); } catch (_) { /* ignore */ }
        }
    }, 20000);
    log.media.info('conference WVP FLV ingress started', { camId });
    return { camId, wvpFlv: true };
}

async function startFromRtsp(sessionKey, rtmpUrl, sourceUrl, options) {
    const fixedCamId = String(options && options.fixedCamId || '').trim();
    if (!sessionKey || !rtmpUrl || !sourceUrl || !fixedCamId) {
        throw new Error('Registered fixed camera source is incomplete');
    }
    if (sessions.has(sessionKey)) stopSession(sessionKey);
    const session = {
        camId: sessionKey,
        fixedCamId,
        videoPort: null,
    ...
    startRtspToRtmp(session, sourceUrl, rtmpUrl, options);
    session.sourceWatchdog = setTimeout(function () {
        if (!sessions.has(sessionKey) || session.pipeDecoded) return;
        log.media.warn('conference fixed camera: no decoded video, stopping session', { cameraId: fixedCamId });
        stopSession(sessionKey, { remote: false });
        if (onRemoteStop) {
            try { onRemoteStop(sessionKey, { reason: 'fixed_rtsp_timeout' }); } catch (_) { /* ignore */ }
        }
    }, 20000);
    log.media.info('conference fixed camera ingress started', {
        cameraId: fixedCamId,
        transport: String(options && options.rtspTransport || 'tcp').toLowerCase(),
    });
    return { sessionKey, fixedCamId, fixedCamera: true };
}
