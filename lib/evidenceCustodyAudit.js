...
function listForFile(fileId, limit) {
    ...
    let rows;
    try {
        rows = siteDb.queryAuditForEvidenceFile(id, likeFile, likeEv, n);
    } catch (err) {
...
function recordMissingObserved(req, file) {
    if (!file || !file.id) return false;
    try {
        auditLog.recordFromRequest(req, 'evidence.storage_missing_observed', {
            target: file.id,
            detail: {
                fileId: file.id,
