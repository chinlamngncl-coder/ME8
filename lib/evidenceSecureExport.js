...
async function createRequest(session, fileId, reason, clientIp) {
    ...
    if (session && session.userId) {
        const pending = await siteDb.countPendingSecureExportsForUser(session.userId, fileId);
        if (pending > 0) throw new Error('An export request for this file is already awaiting review.');
    }
    const requestId = newRequestId();
    const now = new Date().toISOString();
    await siteDb.insertSecureExportRequest({
        requestId,
...
async function approveRequest(requestId, session, clientIp) {
    const req = await siteDb.getSecureExportRequest(requestId);
    if (!req) throw new Error('Request not found.');
    if (req.status !== 'pending') throw new Error('This request has already been reviewed.');
    const file = await siteDb.getEvidenceFile(req.evidenceFileId);
    if (!file) throw new Error('Evidence file not found.');
    const fullPath = evidenceRegistry.resolveFilePath(file);
    if (!fullPath) throw new Error('File is not available on storage.');
    ...
    const updated = await siteDb.updateSecureExportRequest(requestId, {
        status: 'approved',
...
async function denyRequest(requestId, session, denyReason) {
    const req = await siteDb.getSecureExportRequest(requestId);
    if (!req) throw new Error('Request not found.');
    if (req.status !== 'pending') throw new Error('This request has already been reviewed.');
...
async function resolveStreamDownload(requestId, session) {
    const req = await siteDb.getSecureExportRequest(requestId);
...
async function markConsumed(requestId) {
    await siteDb.updateSecureExportRequest(requestId, {
