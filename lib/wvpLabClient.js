...
function isEnabled() {
    if (String(process.env.FM_LAB_WVP || '').trim() === '1') return true;
    /* MOB-APPLY-BACKEND-VIDEO-WVP-HANDOFF-V1 — allow WVP API without Soft Open UI mode */
    return String(process.env.FM_WVP_VIDEO_HANDOFF || '').trim() === '1';
}
...
function requestJson(method, pathWithQuery, opts) {
    ...
    return new Promise((resolve, reject) => {
        const req = transport.request({
            ...
            let raw = '';
            res.on('data', (chunk) => { raw += Buffer.isBuffer(chunk) ? chunk.toString('utf8') : String(chunk); });
            res.on('end', () => {
                let json = null;
...
function rewriteStreamHost(urlStr) {
    ...
    try {
        ...
        /* MOB-APPLY-BACKEND-VIDEO-HANDOFF-STABLE-V1 — prefer ZLM HTTP map port (default 18088) */
        const zlmPort = parseInt(process.env.FM_WVP_ZLM_HTTP_PORT || '18088', 10);
        if (Number.isFinite(zlmPort) && zlmPort > 0) {
            if (!u.port || u.port === '80' || u.port === '443') {
                u.port = String(zlmPort);
            }
