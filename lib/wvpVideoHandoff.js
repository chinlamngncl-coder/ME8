...
/** camId → conference owners; prevents dashboard Stop from killing a VC source. */
const conferenceOwners = new Map();
...
/** Optional dashboard notify after hard stop (server.js wires Socket.IO). */
let stopUiNotify = null;

function setStopUiNotify(fn) {
    stopUiNotify = typeof fn === 'function' ? fn : null;
}

/* SOS-STOP-VIDEO-STOP-RECORD-V2 — after hard-stop (not soft pool-stop) */
let onHardStopComplete = null;
function setOnHardStopComplete(fn) {
    onHardStopComplete = typeof fn === 'function' ? fn : null;
}
function maybeOnHardStopComplete(camId) {
    if (typeof onHardStopComplete !== 'function') return;
    try {
        onHardStopComplete(camId);
    } catch (_) { /* never break stop */ }
}
...
/**
 * Prefer LAN ZLM HTTP port (18088) for **upstream** (server pull).
 * SAME-ORIGIN-MEDIA-PROXY-V1 — never treat /api/lab as upstream; never hardcode :3988.
 */
...
function absolutizeFlv(play, publicHost) {
    ...
    if (preferDirect && play.directFlv && /^https?:\/\//i.test(play.directFlv)) {
        ...
        /* Relative /api/lab/… is a browser path — not an upstream ZLM URL */
        if (/^\/api\/lab\//i.test(play.flvUrl)) {
            url = play.upstreamFlv || play.directFlv || null;
        } else if (/^https?:\/\//i.test(play.flvUrl)) {
            url = play.flvUrl;
        } else if (play.flvUrl.charAt(0) === '/') {
            url = 'http://' + host + ':' + port + play.flvUrl;
        } else {
            url = 'http://' + host + ':' + port + '/' + play.flvUrl;
        }
    }
    ...
    if (!url) return null;
    if (/\/api\/lab\//i.test(url)) return null;
...
async function hardStopOne(camId, opts) {
    ...
    const hadActive = active.has(id);

    active.delete(id);

    try {
        require('./wvpWallListenAudio').stopListen(id);
    } catch (_) { /* ignore */ }

    if (!wvpLab.isEnabled()) {
        maybeNotifyStopUi(id, opts, hadActive);
        maybeOnHardStopComplete(id);
...
function maybeNotifyStopUi(camId, opts, hadActive) {
    if (!opts || opts.notifyUi !== true) return;
    if (!hadActive && opts.notifyEvenIfIdle !== true) return;
    if (typeof stopUiNotify !== 'function') return;
    const reason = opts.uiReason || 'device_bye';
    try {
        stopUiNotify(camId, reason);
        log.media.info('wvp handoff stop ui notify', {
            camId: camId,
            reason: reason,
            path: 'wvp-handoff-stop-ui-parity-v1',
        });
    } catch (_) { /* never break stop */ }
}
    ...
    const upstreamReq = transport.get(url, (upstreamRes) => {
        ...
        pipeline(upstreamRes, res, (err) => {
            if (err && err.code !== 'ERR_STREAM_PREMATURE_CLOSE') {
                log.media.warn('wvp flv-stream pipeline error', {
                    camId: id,
                    message: err.message,
                    code: err.code,
                });
...
async function startPlayOnce(id, publicHost) {
    ...
    const sameOriginMedia = require('./sameOriginMedia');
    const flvUrl = sameOriginMedia.toBrowserFlvUrl(buildFlvStreamProxyPath(id, labWvpToken));
...
function ensurePlay(camId, opts) {
    ...
    return enqueueCam(id, async function () {
        ...
        if (cur && cur.flvUrl && (
            (conferenceOwners.get(id) || 0) > 0
            || (Date.now() - (cur.at || 0)) < reuseMs()
        )) {

            cur.at = Date.now();

            return { ok: true, flvUrl: require('./sameOriginMedia').toBrowserFlvUrl(cur.flvUrl), play: cur.play, reused: true };
...
/**
 * Conference acquisition is durable: reuse an active WVP play regardless of
 * the normal UI reuse timer, and expose only the trusted server-side FLV URL.
 */
function acquireForConference(camId, opts) {
    opts = opts || {};
    const id = String(camId || '').trim();
    if (!id) return Promise.resolve({ ok: false, reason: 'camId_required' });
    if (!isHandoffEnabled()) return Promise.resolve({ ok: false, reason: 'handoff_off' });
    ...
    return enqueueCam(id, async function () {
        cancelPendingStop(id);
        let cur = active.get(id);
        let reused = !!(cur && cur.upstreamFlv);
        if (!reused) {
            const started = await startPlayOnce(id, opts.publicHost);
            if (!started || !started.ok) return started || { ok: false, reason: 'start_failed' };
            cur = active.get(id);
        }
        if (!cur || !cur.upstreamFlv) return { ok: false, reason: 'no_server_flv_url' };
        const owners = (conferenceOwners.get(id) || 0) + 1;
        conferenceOwners.set(id, owners);
        cur.at = Date.now();
        log.media.info('wvp conference source acquired', { camId: id, owners, reused });
        return {
            ok: true,
            sourceUrl: cur.upstreamFlv,
            owners,
            reused,
        };
    });
...
function releaseForConference(camId, opts) {
    opts = opts || {};
    const id = String(camId || '').trim();
    if (!id) return Promise.resolve({ ok: true, skipped: true });
    const prev = conferenceOwners.get(id) || 0;
    const owners = Math.max(0, prev - 1);
    if (owners > 0) conferenceOwners.set(id, owners);
    else conferenceOwners.delete(id);
    log.media.info('wvp conference source released', { camId: id, owners });
    if (owners === 0 && opts.stopIfUnused) return stopPlay(id);
    return Promise.resolve({ ok: true, owners });
}
...
function stopPlay(camId, opts) {
    ...
    if (!id) return Promise.resolve({ ok: true, skipped: true });
    const conferenceOwnerCount = conferenceOwners.get(id) || 0;
    if (conferenceOwnerCount > 0 && !opts.ignoreConferenceOwners) {
        cancelPendingStop(id);
        log.media.info('wvp video handoff stop suppressed', {
            camId: id,
            reason: 'conference_owner',
            owners: conferenceOwnerCount,
        });
        return Promise.resolve({ ok: true, skipped: true, reason: 'conference_owner' });
    }

    /* Default notifyUi false — operator releaseCamStreamWhenUnwatched already emits operator_stop.
     * Pass notifyUi:true for device/webhook paths that need Stopped-by-BWC chrome. */
    const hardOpts = {
        notifyUi: opts.notifyUi === true,
        uiReason: opts.uiReason || 'device_bye',
        notifyEvenIfIdle: opts.notifyEvenIfIdle === true,
    };

    if (opts.immediate) {

        return enqueueCam(id, function () { return hardStopOne(id, hardOpts); });
    ...
    const t = setTimeout(function () {
        ...
        enqueueCam(id, function () { return hardStopOne(id, hardOpts); }).catch(function () { /* ignore */ });
...
function getCachedFlv(camId) {
    ...
    if (!cur || !cur.flvUrl) return null;
    return require('./sameOriginMedia').toBrowserFlvUrl(cur.flvUrl);
}

/** Absolute ZLM FLV for server-side wall listen pull (WALL-AUDIO-PATH-V1). */
function getUpstreamFlv(camId) {

    const cur = active.get(String(camId || '').trim());

    return cur && cur.upstreamFlv ? cur.upstreamFlv : null;
...
module.exports = {
    ...
    acquireForConference,

    releaseForConference,

    stopPlay,

    setStopUiNotify,

    setOnHardStopComplete,
