...
function create(options) {
    ...
    /* PTT-HQ-SINGLE-FLOOR-V1 — one HQ talker per target BWC. A second operator whose
       targets overlap a live HQ floor gets { ok:false, busy:true } instead of cross-talk.
       Holder floors go stale when no ptt-audio frame arrives for HQ_FLOOR_STALE_MS
       (browser died without ptt-stop / disconnect), so a stuck floor cannot block forever. */
    const HQ_FLOOR_STALE_MS = Math.max(floorStaleMs, 5000);

    function endHqFloor(socketId) {
        return hqFloors.delete(String(socketId || '').trim());
    }

    function findHqFloorConflict(socketId, camIds) {
        const at = now();
        let conflict = null;
        hqFloors.forEach((floor, holderId) => {
            if (conflict || holderId === socketId) return;
            if (at - (floor.lastFrameAt || floor.startedAt) > HQ_FLOOR_STALE_MS) {
                hqFloors.delete(holderId);
                pttLog('info', 'hq floor released', { socketId: holderId, reason: 'stale' });
                return;
            }
            const overlap = camIds.filter((c) => floor.camSet.has(c));
            if (overlap.length) conflict = { holderId, overlap };
        });
        return conflict;
    }
    ...
    function beginHqFloor(socketId, camIds) {
        ...
        const targets = normalizeCamIds(camIds);
        const conflict = findHqFloorConflict(id, targets);
        if (conflict) {
            counters.hqFloorBusy += 1;
            pttLog('warn', 'hq floor busy', {
                socketId: id,
                holderSocketId: conflict.holderId,
                overlap: conflict.overlap,
                path: 'ptt-hq-single-floor-v1',
            });
            return { ok: false, busy: true, holderSocketId: conflict.holderId, overlap: conflict.overlap };
        }
        const at = now();
        hqFloors.set(id, {
            camIds: targets,
            camSet: new Set(targets),
            startedAt: at,
            lastFrameAt: at,
        });
        return { ok: true };
    ...
    function touchHqFloor(socketId) {
        const floor = hqFloors.get(String(socketId || '').trim());
        if (floor) floor.lastFrameAt = now();
    }
    ...
    /** PTT-SOS-INCIDENT-AUDIO-V1 — true if cam is on any active SOS proximity PTT team. */
    function isCamInSosTeam(camId) {
        const id = String(camId || '').trim();
        if (!id) return false;
        for (const team of sosTeams.values()) {
            if (team && team.memberSet && team.memberSet.has(id)) return true;
        }
        return false;
    }

    function anyCamInSosTeam(camIds) {
        return normalizeCamIds(camIds).some((id) => isCamInSosTeam(id));
    }

    /* PTT-GROUP-MANUAL-RECORD-V1 — operator-armed evidence for a PTT group (no SOS required). */
    let manualRecordTeam = null;

    function setManualRecordTeam(camIds, meta) {
        const ids = normalizeCamIds(camIds).slice(0, 16);
        if (!ids.length) {
            manualRecordTeam = null;
            return [];
        }
        ...
        manualRecordTeam = {
            ...
            armedBy: m.armedBy || null,
        };
        return ids;
    }

    function clearManualRecordTeam() {
        manualRecordTeam = null;
    }

    function isCamInManualRecordTeam(camId) {
        const id = String(camId || '').trim();
        if (!id || !manualRecordTeam || !manualRecordTeam.memberSet) return false;
        return manualRecordTeam.memberSet.has(id);
    }

    function anyCamInManualRecordTeam(camIds) {
        return normalizeCamIds(camIds).some((id) => isCamInManualRecordTeam(id));
    }

    /* SOS-AUDIO-RECORD-GATE-AND-CALL-V1 — a cam with an OPEN SOS incident is an evidence cam
       even with no PTT team (solo SOS). Ack closes the incident → gate closes by itself. */
    let openSosResolver = null;
    function setOpenSosResolver(fn) { openSosResolver = typeof fn === 'function' ? fn : null; }
    function isCamInOpenSos(camId) {
        const id = String(camId || '').trim();
        if (!id || !openSosResolver) return false;
        try { return !!openSosResolver(id); } catch (_) { return false; }
    }

    function isPttEvidenceCam(camId) {
        return isCamInSosTeam(camId) || isCamInManualRecordTeam(camId) || isCamInOpenSos(camId);
    }

    function anyCamInPttEvidence(camIds) {
        return normalizeCamIds(camIds).some((id) => isPttEvidenceCam(id));
    }

    function getManualRecordSnapshot() {
        if (!manualRecordTeam || !manualRecordTeam.memberSet) {
            return { active: false, team: [], armedAt: null, armedBy: null };
        }
        return {
            active: true,
            team: Array.from(manualRecordTeam.memberSet),
            armedAt: manualRecordTeam.armedAt || null,
            armedBy: manualRecordTeam.armedBy || null,
        };
    }
    ...
    return {
        ...
        activeTeamForSource,
        isCamInSosTeam,
        isCamInOpenSos, /* SOS-AUDIO-PRIORITY-V1 */
        anyCamInSosTeam,
        setManualRecordTeam,
        clearManualRecordTeam,
        isCamInManualRecordTeam,
        anyCamInManualRecordTeam,
        isPttEvidenceCam,
        anyCamInPttEvidence,
        setOpenSosResolver,
        getManualRecordSnapshot,
        snapshot,
    };
}
