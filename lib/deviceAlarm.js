...
function raiseDeviceAlarm(opts) {
    ...
    if (deps.sosIncidents.hasOpenAlarm(camId)) {
        ...
        /* ALARM-MARKER-CONTRACT-V1 — repeat press on an open incident gets its own Investigation pin. */
        if (typeof deps.logLiveSosAlarm === 'function' && typeof deps.sosIncidents.getOpenAlarms === 'function') {
            try {
                const open = (deps.sosIncidents.getOpenAlarms() || []).find((e) => e && String(e.cameraId) === camId);
                if (open) deps.logLiveSosAlarm(open, { mergeAt: new Date().toISOString(), alarmKind });
            } catch (_) { /* non-fatal */ }
        }
    ...
    /* INV-SOS-PIN-AND-GAP-HONESTY-V1 — Investigation red pin (ISO at from ledger entry) */
    if (typeof deps.logLiveSosAlarm === 'function') {
        try { deps.logLiveSosAlarm(incident); } catch (_) { /* non-fatal */ }
    }
    ...
    // Path B: one DeviceControl Record on new alarm only (not on merge — no storm).
    if (typeof deps.scheduleDeviceRecord === 'function') {
        deps.scheduleDeviceRecord(camId, incident.id);
    }
    /* SOS-CASE-ON-RAISE-V1 — file SO- case on raise (never fail alarm). */
    if (typeof deps.ensureOpsCaseOnRaise === 'function') {
        try {
            deps.ensureOpsCaseOnRaise(incident);
        } catch (wireErr) {
            if (deps.log && deps.log.web && deps.log.web.warn) {
                deps.log.web.warn('ops case SOS raise wire skipped', {
                    sosIncidentId: incident && incident.id,
                    err: wireErr && wireErr.message ? wireErr.message : String(wireErr),
                });
            }
        }
    }
