...
function create(options) {
    ...
    function snapshot() {
        if (!active) return { active: false, phase: 'idle', kind: null, participants: [] };
    ...
    function stop(reason) {
        ...
        mediaLog('info', 'group call ended', {
            callId: call.id,
            kind: call.kind,
            reason: reason || 'ended',
    ...
    function handleInviteResponse(session, response) {
        ...
        session.error = null;
        /* CALL-GROUP-DISPATCH-V1 — SOS keeps alert tone; dispatch discussion stays silent */
        session.alertFrames = (active && active.playAlertTone)
            ? SOS_ALERT_FRAMES.map((frame) => Buffer.from(frame))
            : [];
        mediaLog('info', 'group call member connected', {
            camId: session.camId,
            kind: active && active.kind,
            localPort: session.localPort,
            remoteHost: session.remoteHost,
            remotePort: session.remotePort,
            toneMs: session.alertFrames.length * FRAME_MS,
        });
        emitState();
    }
    ...
    async function start(startOptions) {
        ...
        const alarmCamId = String(input.alarmCamId || '').trim() || null;
        const kind = String(input.kind || '').trim() || (alarmCamId ? 'sos' : 'dispatch');
        const playAlertTone = input.playAlertTone != null
            ? !!input.playAlertTone
            : kind === 'sos';
        const call = {
        ...
        if (active === call) {
            ...
            mediaLog('info', 'group call started', {
                callId: call.id,
                kind: call.kind,
                alarmCamId: call.alarmCamId,
                playAlertTone: call.playAlertTone,
