...
function scheduleChecks() {
    ...
    /* Expiry kill switch runs every 6 h regardless of heartbeat URL */
    const expiryCheckMs = 6 * 60 * 60 * 1000;
    const expiryTick = setInterval(function () {
        runCheck().catch(function () {});
    }, expiryCheckMs);
    if (expiryTick.unref) expiryTick.unref();
    if (!url) return;
...
async function runCheck() {
    /* ── Local expiry kill switch (runs even on offline/no-heartbeat-url installs) ── */
    (function enforceLocalExpiry() {
        try {
            const lic = platformLicense.loadAndValidate();
            if (!lic || !lic.valid) return; // already invalid — nothing to kill
            const expiryRaw = (lic.license && lic.license.expiresAt) || '';
            if (!expiryRaw) return;
            const expiryMs = Date.parse(String(expiryRaw).trim());
            if (!Number.isFinite(expiryMs)) return;
            if (Date.now() > expiryMs) {
                platformLicense.invalidateCache();
                log.web.warn('license expired — cache invalidated (kill switch)', { expiryDate: expiryRaw });
            }
        } catch (_) { /* never crash the heartbeat */ }
    })();
