 ...
 * Optional hatch (ANPR-LIVE-CPU-BUDGET-SHARP-EMIT-V1): FM_ANPR_SNAP_HTTP_URL
 * with `{camId}` — HTTP JPEG snapshot instead of ffmpeg demux (must return image/*).
 * @param {string} camId
...
function grabJpegForFr(camId, videoWsPort) {
    ...
    const httpTpl = String(process.env.FM_ANPR_SNAP_HTTP_URL || '').trim();
    if (httpTpl && id) {
        return grabJpegFromHttpTemplate(httpTpl, id).catch(() => {
            /* fall through to FLV/ffmpeg */
            return grabJpegForFrFfmpeg(id, videoWsPort);
        });
    }
    return grabJpegForFrFfmpeg(id, videoWsPort);
}

function grabJpegForFrFfmpeg(camId, videoWsPort) {
    const id = String(camId || '').trim();
    try {
...
/**
 * Optional still via HTTP GET (ZLM/WVP snap URL if operator configures template).
 * @param {string} template
 * @param {string} camId
 * @returns {Promise<Buffer>}
 */
function grabJpegFromHttpTemplate(template, camId) {
    const url = String(template || '')
        .replace(/\{camId\}/gi, encodeURIComponent(camId))
        .replace(/\{CAM_ID\}/g, encodeURIComponent(camId));
    if (!/^https?:\/\//i.test(url)) {
        return Promise.reject(new Error('anpr snap http: bad url'));
    }
    ...
    const t0 = Date.now();
    return new Promise((resolve, reject) => {
        const req = lib.get(url, { timeout: FLV_GRAB_MS }, (res) => {
            const chunks = [];
            res.on('data', (d) => chunks.push(d));
            res.on('end', () => {
                const buf = Buffer.concat(chunks);
                if (res.statusCode >= 200 && res.statusCode < 300 && buf.length > 200) {
                    try {
                        const log = require('./fleetLog');
                        log.media.info('anpr snap http ms', {
                            camId,
                            ms: Date.now() - t0,
                            bytes: buf.length,
                ...
                reject(new Error('anpr snap http: status ' + res.statusCode));
            });
        });
        req.on('error', reject);
        req.on('timeout', () => {
            try { req.destroy(); } catch (_) { /* ignore */ }
            reject(new Error('anpr snap http: timeout'));
        });
    });
}
