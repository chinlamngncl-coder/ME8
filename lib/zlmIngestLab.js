...
/** Short-lived tokens for FLV proxy — mpegts.js does not send session cookies. */
const labFlvTokens = new Map();
const FLV_TOKEN_TTL_MS = 120000;
...
function useFlvProxy() {
    /* Lab or pack — never expose :8080/:18088 to browser/WAN (SAME-ORIGIN-MEDIA-PROXY-V1) */
    return String(process.env.FM_LAB_ZLM || '').trim() === '1'
        || String(process.env.FM_ZLM_PACK || '').trim() === '1'
        || String(process.env.FM_ZLM_SPAWN || '').trim() === '1'
        || String(process.env.FM_ZLM_FLV_PROXY || '').trim() === '1'
        || String(process.env.FM_LAB_WVP || '').trim() === '1'
        || String(process.env.FM_WVP_VIDEO_HANDOFF || '').trim() === '1'
        || String(process.env.FM_HTTPS_ENABLED || '').trim() === '1';
}

/** Direct ZLM HTTP-FLV (server-side / builder debug only). */
function flvUpstreamUrl(streamId) {
...
function issueFlvToken(streamId) {
    const sid = String(streamId || '').trim();
    const token = crypto.randomBytes(12).toString('hex');
    labFlvTokens.set(token, { streamId: sid, exp: Date.now() + FLV_TOKEN_TTL_MS });
    return token;
}

function validateFlvToken(token, streamId) {
    const t = String(token || '').trim();
    const sid = String(streamId || '').trim();
    if (!t || !sid) return false;
    const row = labFlvTokens.get(t);
    if (!row || row.exp < Date.now() || row.streamId !== sid) return false;
    return true;
}

/** Browser play URL — same-origin proxy when Gate B lab is on. */
function flvPlayUrl(streamId, ctx) {
    void ctx;
    const sameOriginMedia = require('./sameOriginMedia');
    if (useFlvProxy()) {
        const sid = String(streamId || '').trim();
        const token = issueFlvToken(sid);
        return sameOriginMedia.toBrowserFlvUrl(
            '/api/lab/zlm/flv/' + encodeURIComponent(sid) + '.live.flv?labFlv=' + token
        );
    }
    return sameOriginMedia.toBrowserFlvUrl(flvUpstreamUrl(streamId));
}
...
/** Gate B B3 — poll ZLM until HTTP-FLV source is listed (≤ maxMs). */
async function waitForStreamPublished(streamId, maxMs) {
    const deadline = Date.now() + (maxMs || 5000);
    while (Date.now() < deadline) {
        if (await isStreamPublished(streamId)) return true;
