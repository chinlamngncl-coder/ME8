...
async function listInbox(opts) {
    ...
    if (!ftpRoot || !fs.existsSync(ftpRoot)) {
        return {
            ok: true,
            ftpRoot: ftpRoot || null,
            files: [],
            items: [],
            count: 0,
            message: 'ftp_root_missing',
        };
    }
...
/**
 * Permanently remove staging files under FTP root (not Evidence Library).
 * @returns {{ ok: boolean, deleted: string[], failed: Array<{rel:string,error:string}> }}
 */
function purgeFiles(ftpRoot, rels) {
    const deleted = [];
    const failed = [];
    ...
    for (const raw of list) {
        ...
        try {
            ...
            failed.push({ rel: hit.rel, error: String(err && err.message || err).slice(0, 80) });
        }
    }
