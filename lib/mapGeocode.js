...
/* AIRGAP-GEOCODE-AND-FETCH-TIMEOUT-V1 — default OFF. Public Nominatim only with
   FM_MAP_GEOCODE_ONLINE=1; a LAN geocoder via FM_MAP_GEOCODE_URL enables itself. */
function isEnabled() {
    if (geocodeDisabled() || offlineOnlyDeploy()) return false;
    if (String(process.env.FM_MAP_GEOCODE_URL || '').trim()) return true;
    return String(process.env.FM_MAP_GEOCODE_ONLINE || '').trim() === '1';
}
