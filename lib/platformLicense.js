...
const platformLicenseModules = require('./platformLicenseModules');
...
function canonicalString(payload) {
    ...
    if (payload.modules) {
        obj.modules = platformLicenseModules.normalizeModules(payload.modules);
    }
...
function validatePayload(raw) {
    ...
    if (sku) {
        if (sku !== 'core' && sku !== 'professional' && sku !== 'enterprise') {
            return { valid: false, error: 'License sku must be core, professional, or enterprise' };
        }
        const maxSuperAdmins = parseInt(raw.maxSuperAdmins, 10);
        const maxOperators = parseInt(raw.maxOperators, 10);
        const evidenceRetentionDays = parseInt(raw.evidenceRetentionDays, 10);
        if (!Number.isFinite(maxSuperAdmins) || maxSuperAdmins < 1) {
            return { valid: false, error: 'License maxSuperAdmins must be a positive number' };
        }
        if (!Number.isFinite(maxOperators) || maxOperators < 1) {
            return { valid: false, error: 'License maxOperators must be a positive number' };
        }
        if (!Number.isFinite(evidenceRetentionDays) || evidenceRetentionDays < 0) {
            return { valid: false, error: 'License evidenceRetentionDays must be 0 or more' };
        }
        payload.sku = sku;
        payload.maxSuperAdmins = maxSuperAdmins;
        payload.maxOperators = maxOperators;
        payload.evidenceRetentionDays = evidenceRetentionDays;
    }
    ...
    const modulesCheck = platformLicenseModules.validateModulesShape(raw.modules);
    if (!modulesCheck.ok) {
        return { valid: false, error: modulesCheck.error };
    }
    if (modulesCheck.modules) {
        payload.modules = modulesCheck.modules;
    }
...
function getEntitlements() {
    ...
    const modules = platformLicenseModules.resolveModules(st.license.modules);
    return {
        licenseId: st.license.licenseId,
        customerName: st.license.customerName,
        type: st.license.type,
        sku: st.license.sku || null,
        maxBwcDevices: st.license.maxBwcDevices,
        maxDashboardUsers: st.license.maxDashboardUsers,
        maxSuperAdmins: st.license.maxSuperAdmins,
        maxOperators: st.license.maxOperators,
        evidenceRetentionDays: st.license.evidenceRetentionDays,
        expiresAt: st.license.expiresAt,
        issuedAt: st.license.issuedAt,
        modules: modules,
        edgeFace: !!(modules.edgeFace && modules.edgeFace.enabled),
        edgeLpr: !!(modules.edgeLpr && modules.edgeLpr.enabled),
    };
}

function getStatusPublic() {
    const st = loadAndValidate();
    const modulesView = platformLicenseModules.publicModulesView(st.license && st.license.modules);
    return {
        ...
        entitlements: st.valid && st.license ? {
            ...
            evidenceRetentionDays: st.license.evidenceRetentionDays,
            modules: modulesView,
            edgeFace: !!(modulesView.edgeFace && modulesView.edgeFace.enabled),
            edgeLpr: !!(modulesView.edgeLpr && modulesView.edgeLpr.enabled),
        } : null,
        modules: modulesView,
