...
async function createCameraIngress(roomName, identity, displayName) {
    ...
    try {
        info = await client.createIngress(IngressInput.RTMP_INPUT, {
            ...
            participantName: displayName || identity,
            // VC-BWC-INGRESS-JERK-SMOOTH-V1 — Fleet ffmpeg already H.264; skip 2nd LiveKit transcode/simulcast.
...
function createBwcIngress(roomName, camId, displayName) {
    return createCameraIngress(roomName, 'bwc-' + camId, displayName || ('BWC ' + camId));
}

function createFixedCameraIngress(roomName, cameraId, displayName) {
    return createCameraIngress(roomName, 'fixed-' + cameraId, displayName || ('Fixed camera ' + cameraId));
}
