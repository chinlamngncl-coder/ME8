...
let callAudioTcpMediaApproved = false;
let callAudioTcpTxIndex = 0;
...
function setupIntercomAudioAfterAnswer(parsed, response, host) {
    ...
    if (parsed.transport === 'tcp') {
        /* Pure voice TCP is the SDK HDA media socket already opened before INVITE. */
        if (callAudioTcpServer) startCallAudioRtpKeepalive();
        else beginIntercomAudioTcp(host, () => startCallAudioRtpKeepalive());
    } else {
...
function stopIntercomAudioListener() {
    ...
    callAudioTcpMediaApproved = false;
    callAudioTcpTxIndex = 0;
...
function drainCallAudioTcpBuf() {
    while (callAudioTcpBuf.length >= hdaMedia.HEADER_SIZE) {
        const flagIndex = hdaMedia.findMediaFlagIndex(callAudioTcpBuf);
        if (flagIndex < 0) {
            if (callAudioTcpBuf.length > 3) callAudioTcpBuf = callAudioTcpBuf.subarray(callAudioTcpBuf.length - 3);
            return;
        }
        if (flagIndex > 0) callAudioTcpBuf = callAudioTcpBuf.subarray(flagIndex);
        const header = hdaMedia.readMediaHeader(callAudioTcpBuf, 0);
        if (!header) {
        ...
        callAudioTcpBuf = callAudioTcpBuf.subarray(header.frameSize);
        if (header.dwCMD === hdaMedia.CMD_MEDIA_INFO) {
            callAudioTcpMediaApproved = true;
            if (callAudioTcpSocket && !callAudioTcpSocket.destroyed) {
                callAudioTcpSocket.write(hdaMedia.buildOpenRet(0));
            }
            log.media.info('call audio hda media info', {
                audio: hdaMedia.parseMediaInfo(header.lpData) && hdaMedia.parseMediaInfo(header.lpData).audio,
            });
            continue;
        }
        if (header.dwCMD === hdaMedia.CMD_AUDIO_DATA && callAudioTcpMediaApproved) {
            handleCallAudioRtpPacket(header.lpData);
            continue;
        }
        log.media.info('call audio hda frame', {
            cmd: header.dwCMD,
            bytes: header.nLength,
            approved: callAudioTcpMediaApproved,
        });
    }
}

function writeCallAudioTcpHda(rtp) {
    if (!callAudioTcpSocket || callAudioTcpSocket.destroyed) return false;
    try {
        const frame = hdaMedia.buildMediaPacket(
            hdaMedia.CMD_AUDIO_DATA,
            callAudioTcpTxIndex,
            rtp,
        );
        callAudioTcpTxIndex = (callAudioTcpTxIndex + 1) >>> 0;
...
function beginIntercomAudioTcp(host, onReady) {
    ...
    activeMedia = { transport: 'tcp', mode: 'audio' };
    callAudioTcpMediaApproved = false;
    callAudioTcpTxIndex = 0;
    ...
    callAudioTcpServer.listen(AUDIO_UDP_PORT, '0.0.0.0', () => {
        log.media.info('call audio listening', { port: AUDIO_UDP_PORT, host, transport: 'tcp-hda' });
        if (onReady) onReady();
    });
}
...
function startOutboundTalkCall(sip, opts) {
    ...
    const inviteBase = Object.assign({
        mode: 'audio',
        audioOnlyCall: true,
        audioDirection: 'both',
        privateHdaTcp: false,
    ...
    const runInvite = (transport) => {
        sendInvite(sip, {
            ...
            ...inviteBase,
            transport,
            intercomProfile: opts.intercomProfile || null,
            onAnswer,
        });
    };
...
function sendInvite(sip, opts) {
    ...
    const inviteSdp = sdpMedia.buildInviteSdp({
        ...
        audioDirection: opts.audioDirection
            || ((opts.useTalkSubject || opts.useVoicePhoneSubject
                || sessionName === 'Talk' || sessionName === 'Phone') ? 'both' : 'recv'),

        voicePhone: !!opts.voicePhone,

        includeTalkExtensions: opts.includeTalkExtensions !== false,

        privateHdaTcp: !!opts.privateHdaTcp,
    ...
    const request = {
        ...
        headers: {
            ...
            subject: opts.useVoicePhoneSubject
                ? sdpMedia.buildVoicePhoneInviteSubject(camId, serverId)
                : (opts.useTalkSubject
    ...
    if (opts.omitSubject) {
        delete request.headers.subject;
    }



    log.media.info('invite sending', {
        camId,
        mode: mode || 'video',
        transport: inviteTransport,
        sessionName,
        voicePhone: !!opts.voicePhone,
        intercomProfile: opts.intercomProfile || null,
        useTalkSubject: !!opts.useTalkSubject,
        useVoicePhoneSubject: !!opts.useVoicePhoneSubject,
        includeTalkExtensions: opts.includeTalkExtensions !== false,
        audioDirection: opts.audioDirection || null,
        subject: request.headers.subject,
        audioOnly: (mode || 'video') === 'audio',
        hasVideoMline: /^m=video/im.test(inviteSdp),
        hasYline: /^\s*y=/im.test(inviteSdp),
        hasFline: /^\s*f=/im.test(inviteSdp),
    });
            ...
            if (response.content) {
                ...
                if (opts.audioOnlyCall && opts.camId) {
                    ...
                    log.media.info('voice call audio-only connected', {
                        ...
                        voicePhone: !!opts.useVoicePhoneSubject,
                        intercomProfile: opts.intercomProfile || null,
                    });
                    if (onAnswer) {
                        onAnswer(response, {
...
module.exports = {
    ...
    setAudioWss,
    setAudioFocusCamId,
    /** WALL-AUDIO-PATH-V1 — WVP handoff listen tee pushes PCM into same audio WSS */
    pushListenPcm: broadcastAudioPcm,
    countActiveStreams,
    startRtspStream,
