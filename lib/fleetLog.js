const fs = require('fs');
const path = require('path');
const siteTime = require('./siteTime');

const LOG_DIR = path.join(__dirname, '..', 'storage');
const LOG_FILE = path.join(LOG_DIR, 'fleet.log');
const RING_MAX = 4000;
const MAX_FLEET_LOG_BYTES = 50 * 1024 * 1024;
const MAX_FLEET_LOG_BACKUPS = 5;
const ROTATE_CHECK_MS = 60 * 1000;

const ringBuffer = [];
let traceEnabled = process.env.FM_TRACE === '1';
let estimatedFileBytes = 0;
let rotating = false;
let rotateTimerStarted = false;

function pushRing(line) {
    ringBuffer.push(line);
    if (ringBuffer.length > RING_MAX) ringBuffer.shift();
}

function stamp() {
    return siteTime.formatLogStamp(new Date());
}

function isBusyError(err) {
    return !!(err && (err.code === 'EBUSY' || err.code === 'EPERM' || err.code === 'EACCES'));
}

function unlinkQuiet(filePath, cb) {
    fs.unlink(filePath, function (err) {
        if (err && err.code !== 'ENOENT' && !isBusyError(err)) { /* ignore */ }
        if (cb) cb();
    });
}

function renameQuiet(from, to, cb) {
    fs.rename(from, to, function (err) {
        if (err && err.code !== 'ENOENT' && !isBusyError(err)) { /* ignore */ }
        if (cb) cb(err);
    });
}

function rotateFleetLogFiles(done) {
    if (rotating) {
        if (done) done();
        return;
    }
    rotating = true;

    const oldest = LOG_FILE + '.' + MAX_FLEET_LOG_BACKUPS;
    unlinkQuiet(oldest, function () {
        let i = MAX_FLEET_LOG_BACKUPS - 1;
        function shiftNext() {
            if (i < 1) {
                renameQuiet(LOG_FILE, LOG_FILE + '.1', function (err) {
                    rotating = false;
                    if (!err) estimatedFileBytes = 0;
                    if (done) done();
                });
                return;
            }
            const from = LOG_FILE + '.' + i;
            const to = LOG_FILE + '.' + (i + 1);
            renameQuiet(from, to, function () {
                i -= 1;
                shiftNext();
            });
        }
        shiftNext();
    });
}

function queueFleetLogRotation() {
    if (rotating) return;
    if (estimatedFileBytes < MAX_FLEET_LOG_BYTES) {
        fs.stat(LOG_FILE, function (err, stat) {
            if (err || !stat) return;
            if (stat.size > estimatedFileBytes) estimatedFileBytes = stat.size;
            if (stat.size >= MAX_FLEET_LOG_BYTES) rotateFleetLogFiles();
        });
        return;
    }
    rotateFleetLogFiles();
}

function startRotateChecker() {
    if (rotateTimerStarted) return;
    rotateTimerStarted = true;
    const timer = setInterval(function () {
        fs.stat(LOG_FILE, function (err, stat) {
            if (err) return;
            if (stat.size > estimatedFileBytes) estimatedFileBytes = stat.size;
            if (stat.size >= MAX_FLEET_LOG_BYTES) queueFleetLogRotation();
        });
    }, ROTATE_CHECK_MS);
    if (typeof timer.unref === 'function') timer.unref();
}

fs.stat(LOG_FILE, function (err, stat) {
    if (!err && stat && Number.isFinite(stat.size)) estimatedFileBytes = stat.size;
});
startRotateChecker();

function appendFleetLine(line) {
    const text = `${line}\n`;
    const nbytes = Buffer.byteLength(text, 'utf8');
    estimatedFileBytes += nbytes;
    if (estimatedFileBytes >= MAX_FLEET_LOG_BYTES) {
        queueFleetLogRotation();
    }
    fs.mkdir(LOG_DIR, { recursive: true }, function () {
        fs.appendFile(LOG_FILE, text, function () {
            /* disk optional — never block emit on log failure */
        });
    });
}

function emit(channel, level, message, detail) {
    let line = `${stamp()} [${channel}] ${level} ${message}`;
    if (detail !== undefined && detail !== null) {
        const extra = typeof detail === 'string' ? detail : JSON.stringify(detail);
        line += ` | ${extra}`;
    }
    pushRing(line);
    try {
        require('../utils/telemetryRecorder').pushLine(line);
    } catch (_) { /* anomaly buffer optional */ }
    try {
        appendFleetLine(line);
    } catch (_) { /* disk optional */ }
    try {
        console.log(line);
    } catch (err) {
        if (!(err && err.code === 'EPIPE' && err.syscall === 'write')) {
            try {
                process.stderr.write(`fleetLog console write failed: ${err && err.message ? err.message : String(err)}\n`);
            } catch (_) { /* output pipe optional */ }
        }
    }
}

function channel(name) {
    return {
        info: (message, detail) => emit(name, 'INFO', message, detail),
        warn: (message, detail) => emit(name, 'WARN', message, detail),
        err: (message, detail) => emit(name, 'ERR', message, detail),
        trace: (message, detail) => {
            if (traceEnabled) emit(name, 'TRACE', message, detail);
        },
    };
}

function isTraceEnabled() {
    return traceEnabled;
}

function setTraceEnabled(on) {
    if (process.env.FM_TRACE === '1') return true;
    traceEnabled = !!on;
    return traceEnabled;
}

function tailRing(opts) {
    const lines = opts && opts.lines ? Math.min(parseInt(opts.lines, 10) || 200, RING_MAX) : 200;
    const channelFilter = opts && opts.channel ? String(opts.channel).trim() : '';
    const contains = opts && opts.contains ? String(opts.contains).trim().toLowerCase() : '';
    let rows = ringBuffer.slice(-lines);
    if (channelFilter) {
        const needle = `[${channelFilter}]`;
        rows = rows.filter((l) => l.includes(needle));
    }
    if (contains) {
        rows = rows.filter((l) => l.toLowerCase().includes(contains));
    }
    return rows;
}

function tailFile(opts) {
    const lines = opts && opts.lines ? Math.min(parseInt(opts.lines, 10) || 200, 2000) : 200;
    try {
        if (!fs.existsSync(LOG_FILE)) return [];
        const raw = fs.readFileSync(LOG_FILE, 'utf8');
        let rows = raw.split(/\r?\n/).filter(Boolean);
        rows = rows.slice(-lines);
        const channelFilter = opts && opts.channel ? String(opts.channel).trim() : '';
        const contains = opts && opts.contains ? String(opts.contains).trim().toLowerCase() : '';
        if (channelFilter) {
            const needle = `[${channelFilter}]`;
            rows = rows.filter((l) => l.includes(needle));
        }
        if (contains) {
            rows = rows.filter((l) => l.toLowerCase().includes(contains));
        }
        return rows;
    } catch (_) {
        return [];
    }
}

module.exports = {
    logFile: LOG_FILE,
    sip: channel('SIP'),
    messaging: channel('Messaging'),
    media: channel('Media'),
    ptt: channel('PTT'),
    ftp: channel('FTP'),
    web: channel('Web'),
    isTraceEnabled,
    setTraceEnabled,
    tailRing,
    tailFile,
};
