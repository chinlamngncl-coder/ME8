...
function flushLearnedCmdsSoon() {
    ...
    learnedCmdsFlushTimer = setTimeout(() => {
        ...
        try {
            ...
            /* JSON-STORE-SINGLE-WRITER-V1 — unique temp + rename (shared helper). Same debounce,
               same sync call; a crash mid-write can no longer leave a torn learned-cmds file. */
            require('./jsonStoreWriter').atomicWriteJsonSync(learnedCmdsPath, out, 2);
        } catch (_) { /* ignore */ }
    }, 400);
}
