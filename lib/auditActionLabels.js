...
const ACTION_LABELS = {
    ...
    'evidence.trim_export_removed': 'Trim copy removed',
    'evidence.trim_exports_cleaned': 'Trim copies cleared',
    ...
    'device.kill_switch_override': 'Kill switch Super Admin override',
...
function formatEvidenceRedactSummary(action, detail) {
    ...
    if (detail.faceFollow || mode === 'face-follow' || mode === 'second-pass-face-follow'
        || action === 'evidence.redact_autoface') {
        parts.push(mode === 'second-pass-face-follow' || detail.secondPass
            ? 'Second pass · auto face tracking'
            : 'Auto face tracking');
    } else if (mode === 'second-pass') {
        parts.push('Second pass on redacted copy');
    } else if (mode === 'static-regions' || mode === 'static') {
        parts.push('Manual marks');
    } else if (mode) {
        parts.push('Redact mode set');
    }
    if ((detail.secondPass || detail.parentExportId)
        && mode !== 'second-pass'
        && mode !== 'second-pass-face-follow'
        && parts.indexOf('Second pass on redacted copy') < 0
        && !String(parts[0] || '').startsWith('Second pass')) {
        parts.push('Second pass');
    }
    ...
    if (action === 'evidence.redact_export_removed' || action === 'evidence.redact_drafts_cleaned'
        ...
        || action === 'evidence.trim_export_removed' || action === 'evidence.trim_exports_cleaned') {
...
function formatDetailSummary(action, detail) {
    ...
    if (String(action || '').indexOf('evidence.redact') === 0
        || action === 'evidence.trim_export_removed'
        || action === 'evidence.trim_exports_cleaned') {
    ...
    if (detail.actorUserId) parts.push('Actor ID: ' + detail.actorUserId);
    if (detail.action && detail.action !== detail.recordCmd) parts.push('Action: ' + detail.action);
