...
/** Free Tier when license.lic is missing or invalid — never unlimited / 5000. */
...
function freeTierCaps() {
    return {
        maxFixedCameras: FREE_TIER.maxFixedCameras,
        maxBwcDevices: FREE_TIER.maxBwcDevices,
        features: Object.assign({}, FREE_TIER.features),
    };
}
...
function canonicalPayload(payload) {
    ...
    /* Optional tier / user-count limits (Tier 1–4 licensing) */
    if (payload.tier != null) obj.tier = parseInt(payload.tier, 10) || null;
    if (payload.superAdmins != null) obj.superAdmins = parseInt(payload.superAdmins, 10) || null;
    if (payload.users != null) obj.users = parseInt(payload.users, 10) || null;
    if (payload.maxMaps != null) obj.maxMaps = parseInt(payload.maxMaps, 10) || null;
    return obj;
}
...
function validateLicenseFile(filePath, opts) {
    ...
    return {
        ...
        trialWildcard: trialWildcard || undefined,
        tier: canon.tier || null,
        superAdmins: canon.superAdmins || null,
        users: canon.users || null,
        maxMaps: canon.maxMaps || null,
    };
}
 ...
 * Lab-open (no license.lic, airgap not required): all commercial features ON (test/dev).
 * Missing/invalid .lic when airgap ship or bad file: Free Tier feature map (analytics OFF).
 * Capacity stays Free Tier caps via checkFixedCamLimit — never unlimited.
 */
function hasFeature(featureName) {
    try {
        ...
        const st = loadAndValidate();
        if (!st.ok || !st.features) {
            return !!FREE_TIER.features[name];
        }
...
/**
 * Capacity: allowed when proposedTotal <= maxFixedCameras (no hardcoded ceiling).
 * Pass Current + New. Deny when Current + New > Max.
 * Missing/invalid license → Free Tier max 16 (never unlimited).
 */
function checkFixedCamLimit(proposedTotal) {
    const n = Math.max(0, Number(proposedTotal) || 0);
    const st = loadAndValidate();
    let max;
    if (!st.ok || !Number.isFinite(Number(st.maxFixedCameras))) {
        max = FREE_TIER.maxFixedCameras;
    } else {
        max = Number(st.maxFixedCameras);
    }
...
/** Pass proposed total (Current + New). Missing/invalid → Free Tier max 10 BWC. */
function checkBwcLimit(proposedTotal) {
    const n = Math.max(0, Number(proposedTotal) || 0);
    const st = loadAndValidate();
    let max;
    if (!st.ok || !Number.isFinite(Number(st.maxBwcDevices))) {
        max = FREE_TIER.maxBwcDevices;
    } else {
        max = Number(st.maxBwcDevices);
    }
    const allowed = Number.isFinite(max) && n <= max;
    return {
        ok: allowed,
...
function getPublicEntitlements() {
    const st = loadAndValidate();
    if (isLabOpen()) {
        const ft = freeTierCaps();
        const labFeatures = Object.assign({}, ft.features);
        Object.keys(labFeatures).forEach(function (k) { labFeatures[k] = true; });
        return {
            licensed: false,
            labOpen: true,
            freeTier: true,
            ...
            maxFixedCameras: ft.maxFixedCameras,
            maxBwcDevices: ft.maxBwcDevices,
    ...
    if (!st.ok || !st.payload) {
        ...
        return {
            ...
            maxFixedCameras: ft.maxFixedCameras,
            maxBwcDevices: ft.maxBwcDevices,
            tacticalPinLiveCap: 8,
            tacticalPlan: null,
            features: ft.features,
            airgapRequired: isAirgapLicenseRequired(),
            errorCode: st.filePresent ? (st.code || 'INVALID') : null,
        };
    }
    return {
        licensed: true,
        ...
        tier: st.tier || null,
        superAdmins: st.superAdmins || null,
        users: st.users || null,
        maxMaps: st.maxMaps || null,
    };
}
