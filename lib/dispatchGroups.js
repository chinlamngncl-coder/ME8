...
const groupsStore = require('./jsonStoreWriter').createStore({
    path: () => groupsPath,
    parse: (data) => (data && Array.isArray(data.groups) ? { groups: data.groups } : null),
    fallback: () => ({ groups: [] }),
});
...
function readFile() {
    ...
    try {
        return groupsStore.read().groups
