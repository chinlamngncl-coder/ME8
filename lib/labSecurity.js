...
function readinessChecklist(settings, deps) {
    ...
    items.push({
        ...
        ok: s.trustProxy,
        detail: s.trustProxy
            ? 'On — enabled automatically when operator login URL uses https://'
            : 'Off — direct HTTP only (LAN default)',
    });
    items.push({
        id: 'metrics',
...
function syncTrustProxyFromOperatorUrl(storageDir, operatorUrl) {
    const url = String(operatorUrl || '').trim();
    if (!url) return load(storageDir);
    const want = url.toLowerCase().startsWith('https://');
    const cur = load(storageDir);
    if (!!cur.trustProxy === want) return cur;
    return save(storageDir, {
        trustProxy: want,
        notes: want
            ? 'Operator login URL uses HTTPS — trust reverse proxy enabled automatically'
            : String(cur.notes || '').trim(),
    });
}
