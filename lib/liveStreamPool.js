...
function setOnSessionUdpBegin(cb) {
    onSessionUdpBeginCb = typeof cb === 'function' ? cb : null;
}

function setOnSessionEnd(cb) {
    onSessionEndCb = typeof cb === 'function' ? cb : null;
}
...
function beginSessionUdpMedia(session, baseDir, mode) {
    ...
    session.rtpListenSocket.bind(session.videoPort, '0.0.0.0', () => {
        ...
        if (onSessionUdpBeginCb) {
            try { onSessionUdpBeginCb(session); } catch (_) { /* ignore */ }
        }
    });
}
...
function endSessionCall(sip, session, done) {
    ...
    const finish = () => {
        ...
        finished = true;
        if (onSessionEndCb) {
            try { onSessionEndCb(session); } catch (_) { /* ignore */ }
        }
        closeSessionRtp(session);
        stopSessionFfmpegOnly(session);
        clearSessionAudio(session);
...
function onRemoteBye(callId) {
    for (const [camId, session] of sessions) {
        if (session.activeDialog && session.activeDialog.callId === callId) {
            ...
            clearSessionAudio(session);
            clearSessionReinvite(session);
            /* Keep session + dashboardActive so live-post-BYE SOS still sends startVideo:false. */
            log.media.info('pool remote bye', { camId, dashboardActive: !!session.dashboardActive });
            return camId;
        }
    }
    return null;
