...
function startSessionFfmpegPipe(session, baseDir, opts = {}) {
    ...
    /* SOS HQ capture: keep audio when present (was -an / mute file). Live mpegts pipe stays -an below. */
    if (session.liveCapturePath) {
        ffArgs.push(
            ...
            '-c:a', 'aac', '-b:a', '64k',
            '-movflags', 'frag_keyframe+empty_moov+default_base_moof', '-y', session.liveCapturePath,
        );
    }
    ffArgs.push(
        '-map', '0:v',
...
/**
 * Phase 1b VMS — Sort streamProfiles by pixel count and return sub/main tokens.
 * Sub-stream (lowest res) → live Command Wall (bandwidth-lean).
 * Main-stream (highest res) → NAS continuous recording (Phase 1c).
 *
 * @param {Array} streamProfiles  Array of { token, name, resolution: { width, height }, encoding }
 * @returns {{ subToken: string|null, subResolution: object, mainToken: string|null, mainResolution: object }}
 */
function pickStreamProfiles(streamProfiles) {
    if (!Array.isArray(streamProfiles) || !streamProfiles.length) {
        return { subToken: null, subResolution: null, mainToken: null, mainResolution: null };
    }
    const sorted = [...streamProfiles].sort(function (a, b) {
        const pixA = (a.resolution && a.resolution.width * a.resolution.height) || 0;
        const pixB = (b.resolution && b.resolution.width * b.resolution.height) || 0;
        return pixA - pixB;
    });
    const sub  = sorted[0];
    const main = sorted[sorted.length - 1];
    return {
        subToken:       sub.token  || null,
        subResolution:  sub.resolution  || null,
        mainToken:      main.token || null,
        mainResolution: main.resolution || null,
    };
}

function startRtspStreamForCam(camId, rtspUrl, baseDir, opts = {}) {
    const id = String(camId || '').trim();
    const url = String(rtspUrl || '').trim();
    if (!id) throw new Error('Fixed camera stream ID required');
    if (!url) throw new Error('Fixed camera RTSP URL required');
    let session = sessions.get(id);
    if (session && session.sourceType !== 'fixed-rtsp') {
        throw new Error('Camera ID is already owned by another live source');
    }
    if (!session) {
    ...
    if (session.ffmpegProcess) return session;
    if (!ffmpegPath) ffmpegPath = resolveFfmpegPath();
    session.sourceType = 'fixed-rtsp';
    session.dashboardActive = true;
    session.baseDir = baseDir;
    session.activeMedia = { transport: 'rtsp', mode: 'video' };

    /* Phase 1b VMS — store main-stream token for Phase 1c NAS writer (do not use for live wall) */
    session.mainProfileToken = opts.mainProfileToken || null;
    if (opts.subProfileToken) {
        log.media.info('fixed camera sub-stream routed to live wall', {
            camId: id,
            subToken: opts.subProfileToken,
            subResolution: opts.subResolution || null,
            mainToken: session.mainProfileToken,
        });
    }

    const transport = opts.rtspTransport === 'udp' ? 'udp' : 'tcp';
    const ffArgs = [
        '-hide_banner', '-loglevel', 'warning',
    ...
    session.ffmpegAttached = false;
    session.ffmpegProcess.on('error', (err) => {
        log.media.err('fixed camera ffmpeg error', { camId: id, message: err.message });
    });
    session.ffmpegProcess.stderr.on('data', () => { /* keep credentials and camera URLs out of logs */ });
    session.ffmpegProcess.on('exit', (code, signal) => {
        log.media.info('fixed camera ffmpeg exit', { camId: id, code, signal: signal || null });
        session.ffmpegProcess = null;
        session.ffmpegAttached = false;
    });
    attachSessionFfmpegToWs(session);
    log.media.info('fixed camera rtsp started', { camId: id, transport });

    /* VMS-NAS-WRITER-V1 — start dedicated main-stream NAS writer (separate process) */
    if (opts.camera && session.mainProfileToken) {
        const vmsNasWriter = require('./vmsNasWriter');
        vmsNasWriter.start(id, opts.camera).catch(function (err) {
            log.media.warn('[pool] nas-writer start failed', { camId: id, error: err.message });
        });
    }
...
function sendSessionInvite(sip, session, opts) {
    ...
    sip.send(request, (response) => {
        ...
        if (response.status === 200) {
            ...
            /* SOS-INVITE-GUARDS-V1 — a late 200 from an earlier INVITE must never overwrite the
               dialog that is already carrying media: ACK it (protocol), BYE it, keep the live one. */
            const live = session.activeDialog;
            if (live && live.callId && live.callId !== response.headers['call-id']) {
                sip.send({ method: 'ACK', uri: cameraContactUri, headers: ackHeaders });
                sip.send({
                    method: 'BYE',
                    uri: cameraContactUri,
                    headers: {
                        ...
                        from: response.headers.from,
                        'call-id': response.headers['call-id'],
                        cseq: { method: 'BYE', seq: (parseInt(response.headers.cseq.seq, 10) || 1) + 1 },
                    },
                });
                log.media.warn('pool stray 200 ignored', {
                    camId, liveCallId: live.callId, strayCallId: response.headers['call-id'],
                    path: 'sos-invite-guards-v1',
                });
                return;
            }
...
function stopStreamForCam(sip, camId) {
    clearCooldownRetry(camId);
    /* VMS: stop NAS writer for fixed cameras before tearing down the session */
    try {
        const vmsNasWriter = require('./vmsNasWriter');
        vmsNasWriter.stop(camId);
    } catch (_) { /* NAS writer may not be running for this cam */ }
    const session = sessions.get(camId);
    if (!session) {
...
function stopAllStreams(sip) {
    /* VMS: stop all NAS writers before tearing down sessions */
    try { require('./vmsNasWriter').stopAll(); } catch (_) { /* ignore */ }
...
function isStreamingForCam(camId) {
    const session = sessions.get(camId);
    if (!session || !session.dashboardActive) return false;
    if (session.sourceType === 'fixed-rtsp') return !!session.ffmpegProcess;
