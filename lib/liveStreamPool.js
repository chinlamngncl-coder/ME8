...
function setOnRtpListening(cb) {
    onRtpListeningCb = typeof cb === 'function' ? cb : null;
}
...
function beginSessionUdpMedia(session, baseDir, mode) {
    ...
    session.rtpListenSocket.bind(session.videoPort, '0.0.0.0', () => {
        ...
        if (onRtpListeningCb) {
            try { onRtpListeningCb(session.camId, session); } catch (_) { /* ignore */ }
        }
    });
}
...
function endSessionCall(sip, session, done) {
    ...
    const finish = () => {
        ...
        finished = true;
        if (onSessionEndCb) {
            try { onSessionEndCb(session.camId, session); } catch (_) { /* ignore */ }
        }
        closeSessionRtp(session);
        stopSessionFfmpegOnly(session);
        clearSessionAudio(session);
