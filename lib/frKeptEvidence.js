...
function keep(opts) {
    ...
    try {
        ...
        return { ok: false, error: 'keep_failed' };
    }
...
const DISCARD_MEDIA_MS = 30 * 24 * 60 * 60 * 1000;

/**
 * Drop JPEG for discarded holds older than 30 days. JSON + disposition log stay.
 */
function purgeExpiredDiscardedMedia(nowMs) {
    const now = nowMs != null ? nowMs : Date.now();
    const cutoff = now - DISCARD_MEDIA_MS;
    let purged = 0;
    let skipped = 0;
    let failed = 0;
    if (!baseDir || !fs.existsSync(baseDir)) {
        return { ok: true, purged: 0, skipped: 0, failed: 0 };
    }
    let names = [];
    try {
        names = fs.readdirSync(baseDir);
    } catch (_) {
        return { ok: false, purged: 0, skipped: 0, failed: 0, error: 'read_failed' };
    }
    for (let i = 0; i < names.length; i++) {
        const name = names[i];
        if (!/^frk-[a-z0-9]+-[a-f0-9]+\.json$/i.test(name)) continue;
        const id = name.replace(/\.json$/i, '');
        const meta = readJsonMeta(id);
        if (!meta || normalizeStatus(meta) !== 'discarded') continue;
        if (meta.mediaPurgedAt) {
            skipped += 1;
            continue;
        }
        const at = Date.parse(meta.dispositionAt || 0);
        if (!Number.isFinite(at) || at > cutoff) continue;
        try {
            const jpg = jpgAbsolutePath(meta.id);
            if (jpg && fs.existsSync(jpg)) fs.unlinkSync(jpg);
            writeJsonMeta(Object.assign({}, meta, {
                mediaPurgedAt: new Date(now).toISOString(),
            }));
            purged += 1;
        } catch (_) {
            failed += 1;
    ...
    return { ok: true, purged: purged, skipped: skipped, failed: failed };
}
