...
const { conferenceShareFileName } = require('./secureId');
...
const fixedIngressStarting = new Set();
...
function init(opts) {
    ...
    bwcOpts.onRemoteStop = function (camId, meta) {
        const handler = String(camId || '').indexOf('fixed:') === 0
            ? handleFixedCameraIngressRemoteStop
            : handleBwcIngressRemoteStop;
        handler(camId, meta).catch(function () { /* ignore */ });
    };
...
function statusPayload() {
  ...
  return {
    ...
    maxCameraIngress: conferenceStore.MAX_BWC_INGRESS,
    openSourceNotice: {
...
async function stopRoomBwcIngressList(room) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if (item.sourceMode === 'wvp-zlm') {
            const refs = liveViewers && liveViewers.countForCam ? liveViewers.countForCam(item.camId) : 0;
            await wvpVideoHandoff.releaseForConference(item.camId, { stopIfUnused: refs === 0 });
        }
    }
    for (let j = 0; j < list.length; j++) {
        const item = list[j];
        const camId = item && item.camId;
        if (camId && item.sourceMode !== 'wvp-zlm') {
            releasePoolStreamWhenUnwatched(camId).catch(function () { /* ignore */ });
        }
    }
}

async function stopRoomFixedCameraIngressList(room) {
    const list = conferenceStore.fixedCameraIngressList(room);
    for (let i = 0; i < list.length; i++) {
        const item = list[i];
        if (!item || !item.sourceId) continue;
        conferenceBwcIngress.stop(item.sourceId);
        if (item.ingressId) {
            try { await conferenceLivekit.deleteIngress(item.ingressId); } catch (_) { /* ignore */ }
        }
    }
...
async function endRoom(roomId, user, perms) {
    ...
    await stopRoomFixedCameraIngressList(room);
    const updated = conferenceStore.upsertRoom(roomId, {
        active: false,
        hostUserId: null,
        hostUsername: null,
...
async function startRecording(roomId, user, perms, opts) {
    assertLivekit();
    ...
    const sosIncidentId = o.sosIncidentId ? String(o.sosIncidentId).trim() : '';
    const rec = {
        id: recordingId,
        roomId,
        ...
        status: 'recording',
        /* VC-SOS-EVIDENCE-BIND-V1 */
        sosIncidentId: sosIncidentId || null,
    };
    conferenceStore.addRecording(rec);
    conferenceStore.upsertRoom(roomId, {
        recording: { recordingId, egressId: out.egressId, startedAt: rec.startedAt, sosIncidentId: rec.sosIncidentId },
    });
...
async function handleBwcIngressRemoteStop(camId, meta) {
    ...
    if (stopped && stopped.sourceMode === 'wvp-zlm') {
        const refs = liveViewers && liveViewers.countForCam ? liveViewers.countForCam(camId) : 0;
        await wvpVideoHandoff.releaseForConference(camId, { stopIfUnused: refs === 0 });
    }
    if (broadcastConferenceUpdate) {
...
async function handleFixedCameraIngressRemoteStop(sourceId, meta) {
    const rooms = conferenceStore.listRooms();
    const room = rooms.find(function (r) {
        return conferenceStore.fixedCameraIngressList(r).some(function (c) { return c.sourceId === sourceId; });
    });
    if (!room) return;
    const stopped = conferenceStore.fixedCameraIngressList(room).find(function (c) {
        return c.sourceId === sourceId;
    });
    const list = conferenceStore.fixedCameraIngressList(room).filter(function (c) {
        return c.sourceId !== sourceId;
    });
    if (stopped && stopped.ingressId) {
        try { await conferenceLivekit.deleteIngress(stopped.ingressId); } catch (_) { /* ignore */ }
    }
    conferenceStore.upsertRoom(room.id, { fixedCameraIngressList: list });
    if (broadcastConferenceUpdate) {
        broadcastConferenceUpdate('conference-fixed-camera-ingress-stopped', {
            roomId: room.id,
            cameraId: stopped && stopped.cameraId,
            reason: (meta && meta.reason) || 'source_stopped',
        });
    }
}

function combinedCameraIngressCount(room) {
    return conferenceStore.bwcIngressList(room).length
        + conferenceStore.fixedCameraIngressList(room).length;
}
...
async function addBwcIngress(roomId, user, perms, camId, displayName) {
    ...
    if (!room || !room.active) throw new Error('Room is not active');
    const list = conferenceStore.bwcIngressList(room);
    if (combinedCameraIngressCount(room) >= conferenceStore.MAX_BWC_INGRESS) {
        throw new Error('Maximum ' + conferenceStore.MAX_BWC_INGRESS + ' camera shares per room');
    }
    ...
    const useWvpHandoff = wvpVideoHandoff.isHandoffEnabled();
    if (!useWvpHandoff && (!liveStreamPool || !liveStreamPool.registerRtpMirror)) {
    ...
    let wvpAcquired = false;
    try {
        if (useWvpHandoff) {
            const source = await wvpVideoHandoff.acquireForConference(camId, {
                publicHost: poolInviteCfg && poolInviteCfg.host,
            });
            if (!source || !source.ok || !source.sourceUrl) {
                throw new Error('WVP conference source failed: ' + ((source && source.reason) || 'no FLV source'));
            }
            wvpAcquired = true;
            await conferenceBwcIngress.startFromWvpFlv(camId, ingress.rtmpUrl, source.sourceUrl);
        } else {
            await ensurePoolStreamForConference(camId);
            await conferenceBwcIngress.startFromPoolMirror(
                camId,
                ingress.rtmpUrl,
                liveStreamPool.registerRtpMirror
            );
        }
    } catch (err) {
        if (liveViewers && liveViewers.removeConferenceRef) {
    ...
    const bwcIngress = {
        ...
        startedAt: new Date().toISOString(),
        startedBy: user.username,
        sourceMode: useWvpHandoff ? 'wvp-zlm' : 'fleet-rtp',
    };
...
async function removeBwcIngress(roomId, user, perms, camId) {
    ...
    for (let j = 0; j < toRemove.length; j++) {
        ...
        if (item.sourceMode === 'wvp-zlm') {
            ...
            releasePoolStreamWhenUnwatched(item.camId).catch(function () { /* ignore */ });
        }
    }
    return { ok: true, bwcIngressList: remaining };
}

async function addFixedCameraIngress(roomId, user, perms, cameraId) {
    assertLivekit();
    if (!canHost(user, perms) && !(perms && perms.conferenceBwcShare)) {
        throw new Error('Camera share permission required');
    }
    const id = String(cameraId || '').trim();
    if (!id) throw new Error('Fixed camera is required');
    const sourceId = 'fixed:' + id;
    if (fixedIngressStarting.has(sourceId) || conferenceBwcIngress.isActive(sourceId)) {
        throw new Error('Fixed camera is already sharing to a conference');
    }
    const camera = fixedCamRegistry.getById(id);
    if (!camera || !camera.enabled) throw new Error('Enabled fixed camera not found');
    if (camera.streamSource !== 'onvif' && camera.streamSource !== 'rtsp') {
        throw new Error('Fixed camera has no playable stream source');
    }
    const room = conferenceStore.getRoom(roomId);
    if (!room || !room.active) throw new Error('Room is not active');
    if (combinedCameraIngressCount(room) >= conferenceStore.MAX_BWC_INGRESS) {
    ...
    const list = conferenceStore.fixedCameraIngressList(room);
    if (list.some(function (c) { return c.cameraId === id; })) {
        throw new Error('This fixed camera is already in the room');
    }
    fixedIngressStarting.add(sourceId);
    let ingress = null;
    try {
        const resolved = await fixedCamOnvif.resolveRegisteredStreamUri(camera);
        const lkName = conferenceStore.livekitRoomName(roomId);
        ingress = await conferenceLivekit.createFixedCameraIngress(lkName, id, camera.name);
        if (!ingress.rtmpUrl) throw new Error('LiveKit ingress did not return RTMP URL');
        await conferenceBwcIngress.startFromRtsp(sourceId, ingress.rtmpUrl, resolved.uri, {
            fixedCamId: id,
            rtspTransport: camera.onvif && camera.onvif.rtspTransport,
        });
        const fixedCameraIngress = {
            cameraId: id,
            sourceId,
            participantIdentity: 'fixed-' + id,
            ingressId: ingress.ingressId,
            displayName: camera.name || id,
            zone: camera.zone || '',
            startedAt: new Date().toISOString(),
            startedBy: user.username,
            sourceMode: 'fixed-' + camera.streamSource,
            audio: false,
        };
        const nextList = list.concat([fixedCameraIngress]);
        conferenceStore.upsertRoom(roomId, { fixedCameraIngressList: nextList });
        return { fixedCameraIngress, fixedCameraIngressList: nextList, ingress };
    } catch (err) {
        conferenceBwcIngress.stop(sourceId);
        if (ingress && ingress.ingressId) {
            try { await conferenceLivekit.deleteIngress(ingress.ingressId); } catch (_) { /* ignore */ }
        }
        throw err;
    } finally {
        fixedIngressStarting.delete(sourceId);
    }
}

async function removeFixedCameraIngress(roomId, user, perms, cameraId) {
    if (!canHost(user, perms) && !(perms && perms.conferenceBwcShare)) {
        throw new Error('Camera share permission required');
    }
    const room = conferenceStore.getRoom(roomId);
    const list = conferenceStore.fixedCameraIngressList(room);
    if (!list.length) throw new Error('No fixed camera share active');
    const id = cameraId ? String(cameraId).trim() : null;
    const toRemove = id ? list.filter(function (c) { return c.cameraId === id; }) : list.slice();
    if (!toRemove.length) throw new Error('Fixed camera share not found');
    for (let i = 0; i < toRemove.length; i++) {
        const item = toRemove[i];
        conferenceBwcIngress.stop(item.sourceId);
        if (item.ingressId) {
    ...
    const remaining = id ? list.filter(function (c) { return c.cameraId !== id; }) : [];
    conferenceStore.upsertRoom(roomId, { fixedCameraIngressList: remaining });
    return { ok: true, fixedCameraIngressList: remaining };
}
...
function buildLobby(user, perms, scope) {
    ...
    return {
        groups: lobbyGroups,
        fixedCameras: fixedCamRegistry.listEnabled().map(function (camera) {
            return {
                cameraId: camera.id,
                sourceId: 'fixed:' + camera.id,
                name: camera.name || camera.id,
                zone: camera.zone || '',
                streamSource: camera.streamSource,
                playable: (camera.streamSource === 'onvif' && (
                    !!(camera.onvif && camera.onvif.host) || !!String(camera.rtspUrl || '').trim()
                )) || (camera.streamSource === 'rtsp' && !!String(camera.rtspUrl || '').trim()),
...
/**
 * Prepare a ready VC recording for Evidence Library ingest (copy into FTP root).
 * Caller runs admit/handleFtpFileUploaded; does not remove the VC source until purge.
 */
function prepareRecordingForEvidencePush(recordingId) {
    const rec = conferenceStore.getRecording(recordingId);
    if (!rec) throw new Error('Recording not found');
    if (rec.status !== 'ready') throw new Error('Only finished recordings can be pushed to Evidence');
    if (!rec.filePath || !fs.existsSync(rec.filePath)) throw new Error('Recording file is missing on this server');
    const base = path.basename(rec.filePath) || (recordingId + '.mp4');
    const safeName = 'vc-' + String(recordingId).replace(/[^A-Za-z0-9_-]/g, '').slice(0, 40) + '-' + base.replace(/[^A-Za-z0-9._-]/g, '_');
    return {
        recording: rec,
        sourcePath: rec.filePath,
        suggestedFileName: safeName,
        displayName: 'Conference ' + (rec.roomId || 'room') + ' · ' + (rec.startedAt || recordingId) + '.mp4',
    };
}

function updateRecordingMeta(recordingId, patch) {
    return conferenceStore.updateRecording(recordingId, patch || {});
}

function bulkDeleteRecordings(ids, user, perms) {
    ...
    for (let i = 0; i < list.length; i += 1) {
        ...
        if (!id) continue;
        try {
            deleteRecording(id, user, perms);
            deleted.push(id);
        } catch (err) {
            failed.push({ id: id, error: String(err && err.message || err).slice(0, 120) });
        }
    }
...
function saveRoomShareImage(roomId, body) {
    ...
    const id = conferenceShareFileName(ext);
...
module.exports = {
    ...
    addBwcIngress,
    removeBwcIngress,
    addFixedCameraIngress,
    removeFixedCameraIngress,
    muteAllParticipants,
    allowSpeak,
    revokeSpeak,
    requestSpeak,
    ...
    streamRecordingFile,
    deleteRecording,
    prepareRecordingForEvidencePush,
    updateRecordingMeta,
