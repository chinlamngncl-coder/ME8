...
const frCaptureHistory = require('./frCaptureHistory');
...
function init(storageDir, options) {
    const opts = options || {};
    const root = storageDir || path.join(__dirname, '..', 'storage');
    try { analyticsCaptureStore.init(root); } catch (_) { /* ignore */ }
    baseDir = path.join(root, 'fr-snap-ledger');
...
function record(opts) {
    ...
    const username = String(opts.username || opts.userId || 'system');
    const deviceLabel = String(opts.deviceLabel || camId);
    const at = opts.at || new Date().toISOString();
    let hierarchical = null;
    try {
        hierarchical = analyticsCaptureStore.saveJpeg('fr', 'micro', buf, {
            username,
            userId: username,
            bwcName: deviceLabel,
            camId,
    ...
    const entry = {
        ...
        at,
        cropFile,
        cropUrl: hierarchical
            ? hierarchical.url
            : cropUrl(cropFile),
        macroPath: null,
        microPath: hierarchical ? hierarchical.rel : cropFile,
        userId: username,
        bwcId: camId,
    ...
    try {
        frCaptureHistory.record(entry).catch(() => { /* ignore */ });
    } catch (_) { /* ignore */ }
    return entry;
}
