...
function testPathAccess(fullPath) {
    ...
    try {
        ...
        out.error = err.code === 'ENOENT' ? 'Folder not found on this server' : 'Access denied';
        if (out.exists && !out.writable) out.error = 'Folder exists but is not writable by Mobility';
    }
    return out;
}

/** Client JSON: never include absolute `path` or raw OS errors. */
function clientSafeAccess(detail) {
    if (!detail || typeof detail !== 'object') {
        return { exists: false, writable: false, error: null };
    }
    const out = {
    ...
    if (detail.ok != null) out.ok = !!detail.ok;
    return out;
}
...
function testWriteLifecycle(fullPath) {
    ...
    try {
        ...
        out.error = 'Storage write lifecycle failed';
    } finally {
