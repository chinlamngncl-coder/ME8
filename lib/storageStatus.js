/**
 * Storage + catalog health — SQLite metadata, evidence paths, NAS validation.
 */
const fs = require('fs');
...
function buildStatus(baseDir, storageDir, settings, opts) {
    opts = opts || {};
    const ftpRoot = storagePaths.resolveFtpRoot(baseDir, settings);
    const lcRoot = storagePaths.resolveLiveCaptureRoot(baseDir, settings);
    ...
    const catalog = siteDb.isReady() ? siteDb.getDatabaseStats() : null;
    ...
    try {
        if (fs.existsSync(backupDir)) {
            const files = fs.readdirSync(backupDir).filter(function (f) { return /\.db$/i.test(f); });
    ...
    return {
        ...
        notes: {
            ...
            postgresPlanned: 'Multi-tenant Phase 3 — FM_CATALOG_DB_URL (not active in V2)',
            videoOnDisk: 'Evidence video files stay on local disk or NAS; SQLite holds catalog metadata only.',
        },
    };
}
