# Snapshot live ME8 after Settings UI Target A PASS (unify + SaaS east-west + file/nav/retire polish).
# AI restores ONLY when user types: RUN RESTORE-ME8-SETTINGS-UI-TARGET-A-PASS
...
foreach ($rel in @(
    ...
    'BASELINE-ME8-SETTINGS-UI-TARGET-A-PASS.md','BASELINE-ME8-CLASSIC-PASS-20260718.md','BASELINE-ME8-PRE-GATE-C.md',
    ...
    'CREATE-ME8-SETTINGS-UI-TARGET-A-PASS.ps1','RESTORE-ME8-SETTINGS-UI-TARGET-A-PASS.ps1','VERIFY-ME8-SETTINGS-UI-TARGET-A-PASS.ps1','CREATE-ME8-PRE-SETTINGS-UI-FALLBACK.ps1','RESTORE-ME8-PRE-SETTINGS-UI-FALLBACK.ps1','VERIFY-ME8-PRE-SETTINGS-UI-FALLBACK.ps1'
...
$skipOverwrite = @(
    'CREATE-ME8-SETTINGS-UI-TARGET-A-PASS-SNAP.ps1',
    'RESTORE-ME8-SETTINGS-UI-TARGET-A-PASS-SNAP.ps1',
    'VERIFY-ME8-SETTINGS-UI-TARGET-A-PASS-SNAP.ps1'
)
...
Write-Host 'CREATE ME8 Settings-UI-Target-A-PASS backup'
...
foreach ($rel in $pathList) {
    ...
    $tier = if ($rel -like 'storage/*') { 'me8-config' } elseif ($rel -eq '.env') { 'me8-env' } elseif ($rel -like 'docs/*') { 'me8-docs' } elseif ($rel -like 'tools/*') { 'me8-tools' } elseif ($rel -like 'db/*') { 'me8-db' } elseif ($rel -like 'android/*') { 'me8-android' } elseif ($rel -like '.cursor/*') { 'me8-cursor' } else { 'me8-settings-ui-target-a-pass' }
...
foreach ($snap in $skipOverwrite) {
    ...
    if (Test-Path $snapPath) {
        ...
        $fileEntries += [ordered]@{ path = $snap; tier = 'me8-settings-ui-target-a-pass' }
        $saved++
...
$manifest = [ordered]@{
    label = 'ME8 Settings-UI-Target-A-PASS backup'
    version = 'me8-settings-ui-target-a-pass-20260726'
    locked = '2026-07-26'
    gitCommit = "$gitHead"
    gitBranch = "$gitBranch"
    note = 'Settings UI Target A PASS floor: unify CSS, SaaS deploy/SSL east-west, file-input + Maintenance divider + BWC Retire theme. AI restores only on RUN RESTORE-ME8-SETTINGS-UI-TARGET-A-PASS.'
    predecessor = 'me8-pre-settings-ui-fallback-20260726'
    purpose = 'Safety net for Settings UI Target A PASS before further SaaS UI work.'
    nextPlanned = 'Further SaaS Steps — UI only after MOB-APPLY'
...
$backupMirror = Join-Path (Split-Path $AppRoot -Parent) 'ME8-BACKUPS\2026-07-26-settings-ui-target-a-pass'
try {
...
Write-Host ''; Write-Host "ME8 Settings-UI-Target-A-PASS lock: $saved files in $OutRoot"
Write-Host 'Next: .\VERIFY-ME8-SETTINGS-UI-TARGET-A-PASS.ps1'
