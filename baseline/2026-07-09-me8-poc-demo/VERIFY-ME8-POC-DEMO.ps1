# Verify live ME8 matches ME8 POC/Demo baseline (SHA256).
...
Write-Host 'VERIFY ME8 POC/Demo baseline'
...
if ($missingLive -eq 0 -and $missingBase -eq 0 -and $mismatch -eq 0) {
    Write-Host 'VERIFY OK - live ME8 matches ME8 POC/Demo byte-for-byte.' -ForegroundColor Green
    exit 0
}
