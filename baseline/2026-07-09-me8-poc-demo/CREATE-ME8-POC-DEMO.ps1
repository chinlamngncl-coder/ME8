# Snapshot live ME8 into ME8 POC/Demo baseline (code + site config).
# Run once after MOB sign-off. Re-run only when user authorizes a new lock.
# Not failed-live, not me8-v2 — demo restore floor only.
...
$paths = @(
    ...
    'UNINSTALL-UBITRON-SERVICE.ps1',
    'START-UBITRON-SERVICE.bat',
    'STOP-UBITRON-SERVICE.bat',
    'START-UBITRON-SERVICE.bat',
    'README-ME8.md',
    'BASELINE-ME8-POC-DEMO.md',
...
Write-Host 'CREATE ME8 POC/Demo baseline'
...
foreach ($rel in $paths) {
    ...
    $tier = if ($rel -like 'storage/*') { 'me8-config' } elseif ($rel -eq '.env') { 'me8-env' } elseif ($rel -like 'data/gis/*') { 'me8-gis' } elseif ($rel -like 'docs/*') { 'me8-docs' } elseif ($rel -like 'docker/*') { 'me8-docker' } elseif ($rel -like 'pack/*') { 'me8-pack' } elseif ($rel -like '.cursor/*') { 'me8-cursor' } else { 'me8-poc-demo' }
...
Write-Host ''
Write-Host "ME8 POC/Demo lock: $saved files in $OutRoot"
Write-Host 'Next: .\VERIFY-ME8-POC-DEMO.ps1'
