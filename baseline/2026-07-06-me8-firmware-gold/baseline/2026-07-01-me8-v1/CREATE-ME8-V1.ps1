# Snapshot live ME8 into me8-v1 baseline (code + site config).
...
Write-Host 'CREATE me8-v1 baseline'
...
foreach ($rel in $paths) {
    ...
    $tier = if ($rel -like 'storage/*') { 'me8-config' } elseif ($rel -eq '.env') { 'me8-env' } elseif ($rel -like 'data/gis/*') { 'me8-gis' } elseif ($rel -like 'docs/*') { 'me8-docs' } elseif ($rel -like 'docker/*') { 'me8-docker' } elseif ($rel -like 'pack/*') { 'me8-pack' } else { 'me8-v1' }
...
$manifest = [ordered]@{
    label = 'Mobility Enterprise 8 BWC (ME8)'
    version = 'me8-v1'
    locked = '2026-07-01'
    gitCommit = '420b354'
    note = 'Pre-ZLK/MVP integration lock. Seeded from Lab 8wc-v2 + ME8 enterprise MOBs. storage/secrets/ not snapshotted. AI must not RESTORE unless user types RUN RESTORE-ME8-V1.'
    predecessor = '8wc-v2'
    frozen = @(
        'lib/psG711Audio.js',
        'lib/pttServer.js',
...
Write-Host ''
Write-Host "me8-v1 lock: $saved files in $OutRoot"
Write-Host 'Next: .\VERIFY-ME8-V1.ps1'
